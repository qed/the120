...
import type { Band, PhaseKey } from "@/app/lib/fp/content/types";
...
/**
 * One normalization for BOTH sides of every email comparison. Generic
 * (trim + lowercase), which is why it OUTLIVED the co-parent invite feature it
 * was first written beside: `fw-guide-core.ts` and `actions/fw-guide.ts` are
 * its live callers now.
 *
 * ── WHAT USED TO BE HERE, AND WHY IT IS NOT ──
 * The second-parent invite (R4): `MAX_PARENTS_PER_FAMILY`, `canInviteCoParent`,
 * `PARENT_INVITE_TTL_MS`, `InviteRecord`/`InviteVerdict` and `inviteVerdict`.
 * v3 plan Unit 10 deleted the invite UI and its action module and turned
 * `/fp/invite/[token]` into a redirect stub, which left this logic with zero
 * live callers — precisely the half-retirement docs/solutions/best-practices/
 * retire-a-feature-by-removing-all-its-surfaces-and-grepping-its-identifiers-
 * to-zero warns about. `path_parent_invites` has never held a row in
 * production, so no family is stranded and no backfill was owed. Removed here
 * rather than left as a decoy; the identifiers grep to zero.
 *
 * ⚠ NOT THE SAME THING as `fw-access-rules.ts`'s guide invites
 * (`FW_GUIDE_INVITE_TTL_MS`, `fwGuideInviteVerdict`). That is a LIVE Founders
 * Weekend concept that merely shares the word "invite", and it stays.
 */
