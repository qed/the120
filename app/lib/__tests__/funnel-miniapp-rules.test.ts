...
describe("the skin swap", () => {
  ...
  it("swaps by COMPLETE literal class names at the root — no CSS-variable override", () => {
    ...
    // The consumer half of this pin (MiniAppShell reading the map, and the
    // Decision 10 no-op CSS-variable trap not appearing in its subtree)
    // retired with the v2 shell — v3 Unit 9 moved it to
    // `archive/new-user-v2/child/[childId]/`. The complete-literal rule the
    // map itself must satisfy is asserted above and still binds every future
    // consumer.
  });
...
/* RETIRED (v3 plan Unit 9): three source-scan describes — "the shell's
 * wiring", "the locked shell", and "the door-change confirm dialog". All three
 * read app/start/child/[childId]/{MiniAppShell,page}.tsx off disk and asserted
 * on its TEXT; that surface is now archive/new-user-v2/child/[childId]/. The
 * RULES they surrounded (resolveStep, initialStepForFacts, isEditLocked,
 * doorChangeNeedsConfirm, changeDoorCore's CAS) are all still exercised above
 * against the fake store — what retired is the pin on the one client that
 * consumed them, not the behaviour. */
