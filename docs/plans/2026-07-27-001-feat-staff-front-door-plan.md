...
- [x] **Unit 8: Read-side enforcement and the write-path guard table** — landed 2026-07-27. Four commits on `feat/staff-front-door-unit-8`. **112 files / 3079 tests** (includes Lane B's funnel Unit 4 part one, rebased onto), `tsc`/`next build`/eslint clean. No migration. **The board door closes as a property of the READ**: the archived fence sits in `resolveFwBoardToken` (the plan's placement — only its refusal becomes the bare 404 that CLEARS the projector frame; a `loadFwBoard` failure holds it), one `path_cohorts` read per poll, fail-closed both ways, tested by setting `archived_at` in SQL WITHOUT revoking (read-property, not side-effect). `loadFwBoardShell` nullable and gated. **Every guard-table row landed as an explicit tested verdict** — REFUSE: mint (Unit 7), board read+shell, CSV import at `runFwImportChunk` (WHOLE-CHUNK, the deferred decision decided: one cohort-level fact ≠ N fabricated row outcomes), link-student, add-guide-grant (targeted read, NOT a widening of `loadFwCohort`); PROCEED, each named for its reason: revoke-grant (de-escalation), token revoke (inert), anonymize (the privacy obligation outlives the weekend), replay-reject and import-exception resolution (closing a tombstone is not roster-building), and the two structurally-invisible defaults — check-in LANDS, quick-create SUCCEEDS — pinned structurally (their cores and harnesses cannot even represent archive state) with the import test also pinning the guard's PLACEMENT. **Review (correctness 4, standards 5, testing 3):** the import gate's read-blip collapse into "cohort is gone" was the exact B4 collapse two units of work exist to remove — the refusal union gained `unavailable`, and the not-found copy stopped being the staff-only sentence shown to a caller who had just passed the staff gate; the shell's fail-closed flip was OVER-closed and the review settled the three-way contract (degrade on a blip over a fact `resolveFwBoardToken` just proved; refuse only on a read that ANSWERED); the timing side-channel on the fence is accepted AND reasoned in the docblock; **the invariant scan was widened to the shared authz gate** — `loadFwCohort`'s select pinned BY VALUE to "id, kind, slug", because the unscanned gate was the single most natural regression site; the chunk-placement scan's anchor became statement-start. **Mutations M25–M29 (each fence deleted), all reddened.** The dry-run checklist gains the client frame-CLEARS entry (the untestable half). Compound: item 7 on the tri-state doc — fail-closed can over-close; a blip on a fact just proven is not an unknown (75 docs). **Carried:** the CLI-level mint test stays core-level by the plan's own by-construction note; behavioral lands-on-archived remains structural (the harnesses cannot represent the state — recorded as the mechanism, not a gap).
...
- [x] **Unit 9: Archive-aware list reads and ops surfaces** — landed 2026-07-27. Four commits on `feat/staff-front-door-unit-9`. **112 files / 3096 tests**, tsc/build/eslint clean, no migration. `listFwOpsCohorts` filters archived BEFORE the count fan-out with `includeArchived` as the explicit widened view (R3's counts tested ON an archived row — the review's decoupling mutation reddens); `listFwCohortsForActor` gains `includeArchived` + `archivedAt` per row **and pagination** (fetchAllRows, order-then-range, truncation refuses); `listFwActiveWeekends` is the hub's one narrow typed-failure read (Unit 11's input). Callers decide, pinned as wiring AND consumption (the layout takes everything — find-by-id header, strict `> 1` canSwitch; the picker splits `!isStaff`): a guide holding only an archived cohort is redirected INTO it; staff defaults hide archived with `?archived=1` as the shareable widened view. The archived ops page renders in ARCHIVED MODE (not 404): banner with a THREE-state actor (unrecorded = the fact is absent, the launch state of all four backfills; **unresolvable** = a recorded actor whose bounded email lookup failed — the review caught the two-state version converting a blip into a false claim, and caught the lookup itself being the unit's one BARE Admin API call), Unarchive control, and `fwOpsCohortAffordances` as the tested rendering twin of Unit 8's guard table — roster-building hidden (and already refused server-side), obligations/de-escalation kept as TYPED literal `true`, the board-token panel unconditional (the frontend-races lesson). `FwArchiveControl` confirms by typed slug; `fwSlugTakenCopy` names the archived list as a tested string. **Mutations M30–M35 all reddened**, including the combined M31 (two redundant filter layers deleted together — each alone surviving is the redundancy working). Compound: tri-state doc item 8 (absent-fact vs failed-lookup need different words). The dry-run checklist gains the archive-control round-trip. **Carried:** the hub-count half of scenario 6 lands with Unit 11's own tests; layout header/canSwitch stay scan-pinned (no jsdom — recorded as the mechanism).
...
- [x] **Unit 10: Retire the four production cohorts** — landed 2026-07-27. One data migration, `20260807120000_fw_archive_rehearsal_cohorts.sql` (the plan's `20260805130000` name was TWO Lane-B collisions stale). **The plan's "tokens are already all revoked" claim was stale on arrival**: PRE-APPLY found a live token on `rehearsal-unit9`, minted by a later rehearsal — so the migration revokes-then-archives IN SQL, the core's own ordering, and the revoke statement is load-bearing. `archived_by`/`revoked_by` NULL (a migration is not a human; Unit 9's banner renders this as unrecorded). WHERE-guarded on slug; idempotent both statements; rollback restores visibility only, tokens stay revoked (matching `unarchiveFwCohort`'s documented semantics — verified by review). Applied via the Management API, POST-APPLY green: 0 active fw cohorts, 0 live tokens on the four, 0 attributed archives, `npm run fw -- cohorts` empty (and `--json` parses). **R21 holds: the hub's first factual claim is true before Unit 11 ships it.** **The migration lock handed BACK to Lane B in the same diff** (Lane A held it Units 6–10 per Peter's Unit-6 approval), with the 2026-07-27 breach noted in the lock file; the review's one finding — LANES.md's flat "Lane B is the sole author" rule now points at the lock file as authority instead. Residual (recorded, inert today): the SQL revoke leaves `revoked_by` NULL where app paths always set it — `revoked_by` is write-only in current code; flag if an attribution report ever assumes non-null.
