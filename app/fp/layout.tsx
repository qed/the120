 ...
 * file convention as `app/fp/apple-icon.png` (this segment), replacing the
 * root "120" badge for /path pages; its URL is allowlisted in proxy-rules.
 */

export const metadata: Metadata = {
  // The manifest FILE keeps its /path.webmanifest name after the /fp rename: the
  // href is never user-visible (only the manifest's own `name` field shows on a
  // home screen, and that is "First Profit"), so renaming the file is churn for
  // zero user value. Its `id`/`start_url`/`scope` inside ARE now /fp.
