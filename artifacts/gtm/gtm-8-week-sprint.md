...
Borrowed from founders.school's "Year in Detail": (1) **named phases with one verb each** — their year is five 8-week sessions named Sell/Build/Validate/Grow/Scale; this sprint is four 2-week phases named **Arm / Seed / Surge / Land**; (2) **"the rhythm stays constant"** — one primary push per week, same weekly cadence throughout; (3) **peer-driven advancement** — their students are voted forward by their cohort; our ambassador program makes kids the engine, not the audience; (4) **parents attend the Demo Days** — every stretch ends on a Demo Day (the first, Nov 7, 2026) where kids present their business on stage; (5) **high-conviction risk reversal** — their "$1M by senior year or full refund" becomes our honest version: *"$250, fully refundable until Sept 30. The only thing you can lose is the seat."*

---

## 0. Change Log — 2026-07-20 (Week 2 review)

*This revision folds in the Week 1 results and a product reframe. The prior version is preserved as `gtm-plan-original.md`.*

**Product reframe (applies throughout):**
- **Cadence:** the four quarterly intensives are replaced by **~20 in-person weekend workshops** (Saturdays, roughly every two weeks, Sept 19 2026 → June 2027). Each stretch ends on a **Demo Day** where kids present their business on stage for the cohort and parents. The **first Demo Day is Nov 7, 2026** — the new anchor everywhere the old "Nov 7–8 intensive" appeared.
- **Throughline:** the product is now **build a real business + get solid with math to run the numbers**, per group — Athletes → NIL, Founders → a company, Makers → shows/exhibits, Scholars → a research lab, Givers → a community-service org.
- **Structure:** **two age cohorts (8–13 and 14–17)**; the program arc is **First Profit — Sell → Build → Validate → Grow → Scale** (5 criteria per phase).

**Week 1 outcome (vs. plan):** machine shipped — booking, live Stripe (a real $250 deposit was taken), domain, and the signup "how did you hear / referral" field. Warm network converted **~80% message→call** (26 messaged → 16 conversations → 8 calls held). **33 families, 2 dossiers, 1 deposit** — ahead of plan, since deposits weren't due until W4. Misses: ambassadors at 0, and the welcome email (now shipped). New signal: **3 inbound strangers from the founder's X posts** (unplanned, free). Full detail in `The120-GTM-Tracker.xlsx`.

**Week 2 adjustment (see the W2 row):** the primary push shifts from "recruit 12–15 ambassadors" to **protect/expand the warm engine (+10 families)** plus **personally seed 5 ambassadors** (stretch 12–15 if Ethan leads; the big push may slide to W3 — decide Monday). Rationale: warm is converting strongly and is the highest-ROI lever, while ambassadors were at zero and their core tool (the Gauntlet share card) isn't built yet.

**New assets produced (all in the `gtm` folder):** GTM tracker workbook; finalized one-pager (content + branded PDF); welcome email (copy + branded HTML + plain-text + fire-on-signup trigger spec); ambassador kit; ambassador kickoff run-of-show; Gauntlet share-card design brief; CRM pipeline-stages spec.
...
**Blocking dependency (Week 1, non-negotiable):** Stripe is still test mode (roadmap S10). No real deposit can be taken until live keys + a verified $250 charge/refund round-trip ship. Also: booking link (T2) doesn't exist, and there's no email provider. Week 1 exists to fix all three. **[Update 2026-07-20 — resolved: booking link live, Stripe live (a real $250 deposit was taken), Resend + the120.school domain live; welcome email shipped in W2.]**
...
|---|---|---|---|---|---|
| **W1** Jul 13–19 | **Machine online + warm 25** | Create Cal.com booking link (T2, 30 min) and set `NEXT_PUBLIC_BOOKING_URL`. Kick S10 Stripe go-live (account decision + live keys + one real charge/refund test). Pick email provider (Resend for transactional + simple broadcasts; Customer.io if budget allows) and wire welcome email. Dev ticket: "How did you hear about us? / referral code" field on account creation. Personally message 25 warmest contacts (UTS circles, the 7 founding families, /parents story families) — personal notes, not blasts. | Personal email/text, coffee | 25 warm convos started, 10 calls booked, machine live | **One-page explainer PDF** (five groups, $3,000, refundable $250, 20 weekend workshops) + welcome email #1 |
| **W2** Jul 20–26 | **Protect the warm engine + seed ambassadors** *(adjusted — see §0)* | Warm engine is the primary push: message the ~6 unsent, rebuild the warm list past 25, and ask each W1 warm family for two introductions (goal **+10 families**). Personally seed **5** Founding Ambassadors (stretch 12–15 if Ethan leads); run one 45-min kickoff this week. Ship the Gauntlet share-card spec + CRM pipeline-stages spec to Ethan. Welcome email #1 now live; one-pager finalized. **Decision Mon:** if the 12–15 push isn't standing up, it slides to W3. | Personal notes/text, warm intros, one Zoom kickoff | +10 warm families (~43 cum.), 5 ambassadors seeded, 20 calls booked cum. | **Ambassador kit + kickoff run-of-show** (done) · **Gauntlet share-card brief + pipeline-stages spec** (to Ethan) · **welcome email HTML + fire-on-signup trigger spec** |

*Week 1 actual (vs. plan): machine live — booking, live Stripe, domain, signup source field. Warm outreach converted ~80% message→call (26 messaged → 16 conversations → 8 calls held). 33 families, 2 dossiers, 1 deposit — deposits weren't planned until W4. Ambassadors: 0 (the week's miss). New signal: 3 inbound strangers from the founder's X posts (unplanned, free). Full detail in `The120-GTM-Tracker.xlsx`.*
...
| **W4** Aug 3–9 | **The Gauntlet goes public** | Open the Gauntlet Summer Tournament to everyone: 3-week leaderboard sprint, grade bands 3–4/5–6/7–8, weekly "boss week" themes. Winners earn a named spot on a permanent Founding Leaderboard + first-demo slot at the Nov 7 Demo Day. Ambassadors challenge classmates; parents share scores. Landing: /gauntlet with a parent-facing banner → join flow. Dev ticket: shareable score card image. | Kid word-of-mouth, ambassador shares, parent groups ("free math game a Toronto founder built") | +35 interested (105 cum.), 40 calls cum., first 15 deposits | **Gauntlet share card** (auto-generated score image) + tournament rules page + weekly leaderboard email |
...
- **Founding Ambassador** title: named in the Address Book, badge at the Nov 7 Demo Day.
- **Gauntlet glory:** permanent leaderboard flair + early access to new bosses (cheap dev ticket, huge to a 12-year-old).
- **Demo Day perks:** first pick of demo slot, ambassador table at the Nov 7 Demo Day.
- Milestone recognition (3 families joined → Demo Day shout-out), not per-head bounties.
...
6. **Demo Day #1 (Nov 7, 2026) sellable:** date + city confirmed in all copy, venue at least internally locked.
...
| Deposit paid | Welcome to the Founding 120 → Demo Day #1 details → "know one more family?" referral note |
...
## 6b. Do This Monday (Jul 20 — Week 2)

- [ ] Message the ~6 unsent warm contacts + add new names to push the list past 25.
- [ ] Ask each W1 warm family (the 8 who talked) for two introductions.
- [ ] Confirm Ethan on the head-ambassador role (know by Tue): if yes, scale toward 12–15; if no, run the kickoff with your 5.
- [ ] Personally seed the 5 Founding Ambassador families; schedule the 45-min kickoff for a weekday evening this week.
- [ ] Hand Ethan the Gauntlet share-card design brief + CRM pipeline-stages spec.
- [ ] Load welcome email #1 into Resend (HTML + text), send yourself a test, wire the fire-on-signup trigger.
- [ ] Circulate the finalized one-pager to the warm list.
- [ ] Friday: update `The120-GTM-Tracker.xlsx` and review vs. targets.
...
4. **Demo Day #1 venue:** Nov 7, 2026 is confirmed (per the live /2026-27 page). Lock the venue internally — it's the closing argument in every call and email, and "details on enrollment" gets weaker as September nears.
