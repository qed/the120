...
**GTM-3 · Summer Tournament shell** *(dev)* — 🟡 **Marketing/website layer built dormant 2026-07-16 → see the GPF series below.** The public front door (nav, homepage section w/ states, /gauntlet banner, entry gate + double opt-in, rules page, weekly-theme highlight, standings email, Founding Leaderboard page) all ship behind the `app/lib/tournament.ts` state machine and flip on with one switch. **🟡 Phase-1 spine SHIPPED 2026-07-17 (PR #11):** B6 account-to-rank linkage + B5 `g912` band & prize-band mastery leaderboard + B1 mastery-event integrity — built, reviewed, migrations applied to production (dormant). Scoring = **difficulty-weighted mastery** (re-demonstrable for cash; see gauntlet-roadmap Decision 8 + `docs/plans/2026-07-17-001-…`). **Still needs before Aug 3:** Phase-2 content (new fraction/expression answer engines + broader HS kernels), cap-number tuning, the age-bracket policy call, operational turn-on (`TOURNAMENT_STATE=live` for W2 + Vercel BotID/Firewall on the scoring routes as a go-live blocker + the winner re-demonstration process), B2 handle filter, `CRON_SECRET`/`STANDINGS_ENABLED`. Full decomposition: `artifacts/gauntlet-public-front-door-stories.md`; brief: `artifacts/public-gauntlet-marketing.md`.
...
### 📋 Tournament go-live — status ledger (2026-07-17, resume here)

**Scoring model (locked, Decision 8 in `gauntlet-roadmap.md`):** the tournament ranks by **difficulty-weighted mastery** — distinct facts mastered during the window (mastered = correct <3 s twice, the M5 model), weighted by content band. Chosen over cumulative-XP because mastery is **re-demonstrable for cash** (a winner re-answers a sample of mastered facts live), grind-proof (one credit per fact), and reuses the M5 model + A4 heatmap. Prize bands = self-declared **age brackets**; content is fully open. Full plan: `docs/plans/2026-07-17-001-feat-gauntlet-tournament-golive-spine-plan.md`.

**✅ DONE — Phase-1 spine, shipped to `main` (PR #11, dormant; reviewed + fixed):**
- **Migrations applied to production + verified (dormant):** `gauntlet_tournament_entries` (had shipped dormant in PR #9 but was never applied — done now) + `gauntlet_tournament_scoring` (`gauntlet_tournament_events` append-only mastery audit log with `unique(user_id,fact_key)` = first-master-once/replay-safe; windowed prize-band `gauntlet_tournament_leaderboard(prize_band, window)` RPC; **partial unique index so one account can't rank in >1 prize band** — a review P0). Both recorded in `schema_migrations`.
- **Content (B5 slice):** `g912` content band + 9 high-school topics on the existing numeric/choice engines (slope, linear/quadratic eval, exponent quotient rule, discriminant root-count, distance, 30-60-90, simplify-radical, midpoint); `bandMasteryWeight` (g34<g56<g78<g912) in `app/gauntlet/game/masteryWeight.ts`.
- **Integrity (B1):** `POST /api/gauntlet/tournament/mastery` — the only path that credits mastery; gated on phase=live + a confirmed entry + verified session; plausibility caps (rate window tracks real elapsed play); never trusts a client score. Audit rows in the events table. Rules copy → **live re-demonstration** (`app/gauntlet/rules/page.tsx`).
- **Linkage (B6):** `AccountModal.onAuthed` callback; entry drives account creation for guests; enter route additively stamps `user_id` from the verified session only (**shipped hardening R7 intact — 0 deletions**); `POST /api/gauntlet/tournament/reconcile` links returning confirmed entrants by **proven email only** (handle-claim was cut — it was hijackable via public handles).
- **Surface:** `FoundingBoard` renders the three prize/age-band pools by mastery ("N facts mastered"), handles-only.
- **Quality:** 493 unit tests, `tsc`, and `next build` all clean; 8-angle code review done (closed a reconcile-handle-claim hijack + a rate-cap under-credit bug).

**🔴 NOT DONE — remaining before Aug 3 (the resume list):**
1. **Phase-2 content (R3 "full depth"):** build the new **fraction / short-expression / two-numbers answer-input engines** (roadmap C6) + broader 9–12 kernels from `gauntletcontent.md`. Phase 1 shipped 9–12 on numeric/choice only. Set a concrete kernel-count floor per engine (don't march the whole 346-entry taxonomy).
2. **Cap tuning:** the mastery rate/daily ceilings (`app/lib/gauntlet/masteryCaps.ts`) ship with placeholder defaults — tune against real W2 play traces.
3. **Age-bracket policy:** self-declaration is unverified — decide: verify age at payout (winners are re-demonstrating anyway), gate multiplier to declared bracket, or accept + document.
4. **Operational turn-on:** `TOURNAMENT_STATE=live` on the W2 soft-launch env (routes 403 until phase=live); **Vercel BotID/Firewall on `/api/gauntlet/tournament/{mastery,enter}` as a GO-LIVE BLOCKER** (self-reported caps are the only other defense on a cash board); define the **winner re-demonstration process** (who reviews, pass/fail, appeal); `CRON_SECRET` + `STANDINGS_ENABLED`; **B2** handle word/profanity filter.
5. **Live E2E dry-run:** with phase=live on a preview — sign up → enter → play → master facts → appear on the prize-band board; confirm no `user_id` ranks in >1 band, mastery POSTs credit, reconcile links a returning entrant.
6. **Standings email:** now unblocked — wire the real per-band rank once the board is populated (was blocked on the ranking design).
7. **Residual review follow-ups (minor):** enter route stamps `user_id` on a *pending*-handle refresh (narrow link vector; pending handles aren't public); reconcile returns `linked:true` on a 0-row concurrent update (cosmetic); cap-reason label ties `daily` vs `rate` (audit string only).

**Files to start from:** `app/gauntlet/game/problems.ts` (add engines/kernels), `app/lib/gauntlet/masteryCaps.ts` (tuning), `app/api/gauntlet/tournament/{mastery,reconcile,enter}/route.ts`, `app/gauntlet/GauntletGame.tsx`, `app/gauntlet/components/FoundingBoard.tsx`, `app/lib/tournament.ts` (phase/bands). Plan has per-unit detail + test scenarios.
...
## 🛤 First Profit (app at `/fp`) — deferred gates
