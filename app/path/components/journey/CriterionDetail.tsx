...
import { Crest } from "@/app/path/components/system/Crest";
import { HQTaskCard } from "@/app/path/components/hq/HQTaskCard";
import { Icon } from "@/app/path/components/system/Icon";
import { TrailStep } from "@/app/path/components/trail/TrailStep";
      ...
      <div className="mb-4 flex justify-between gap-1.5">
        {tasks.map((t) => (
          <TrailStep
            ...
            phase={phaseKey}
            label={t.title}
            onClick={t.state === "locked" ? undefined : () => router.push(`/path/task/${t.id}`)}
          />
        ))}
      </div>
      ...
      <div className="mt-5 flex flex-col gap-3">
        {tasks.map((t) => (
          <HQTaskCard
            ...
            onOpen={t.state === "locked" ? undefined : () => router.push(`/path/task/${t.id}`)}
          />
        ))}
      </div>
