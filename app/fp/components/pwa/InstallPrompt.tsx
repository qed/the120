...
import type { DurabilityWarning } from "@/app/fp/lib/sync-rules";
  ...
  return (
    ...
    >
      <div className={cn("mb-1 flex items-center gap-2 font-path-body text-[13.5px] font-semibold", trail ? "text-trail-ink" : "text-hq-ink")}>
        ...
        {urgent ? "Your captures aren't safe on this iPhone yet" : "Add First Profit to your Home Screen"}
      </div>
      <p className={cn("font-path-body text-[12px] leading-snug", trail ? "text-trail-ink-soft" : "text-hq-ink-soft")}>
        {urgent ? (
          <>
            ...
            browser storage after about a week away. Install First Profit to keep it safe — or get back
            online so it can send now.
          </>
        ) : (
          <>Installed, your work is safe on this device even offline — and First Profit opens like an app.</>
        )}
      </p>
      ...
      {sheetOpen ? (
        <ol className={cn("mt-3 space-y-2 rounded-xl border p-3 font-path-body text-[12.5px]", trail ? "border-trail-ink/12 bg-trail-canvas text-trail-ink" : "border-hq-border bg-hq-surface text-hq-ink")}>
          ...
          <li>
            <b>3.</b> Tap <b>Add</b> — then open First Profit from your Home Screen
          </li>
        </ol>
      ) : null}
