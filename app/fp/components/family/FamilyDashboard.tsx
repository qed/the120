...
import { unwrapActionResult, type UnwrappedResult } from "@/app/fp/lib/now-card-rules";
import type { FounderCard as FounderCardData } from "@/app/fp/lib/onboarding-rules";
import { resetStudentPasswordAction } from "@/app/fp/lib/actions/provision";
import { inviteCoParentAction, resendInviteAction } from "@/app/fp/lib/actions/invite";
    ...
    <div className="rounded-2xl border border-hq-border bg-hq-canvas p-4 shadow-hq">
      ...
      {card.stranded ? (
        <p className="mt-3 rounded-lg border border-not-yet/40 bg-not-yet/10 p-3 font-path-body text-xs leading-5 text-hq-ink">
          {card.firstName}&apos;s First Profit account is still being set up — check back soon, or contact The 120 if
          this persists.
        </p>
      ) : (
        <>
