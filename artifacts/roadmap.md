...
Remaining: (a) T+2d/T+5d/T+9d account-created sequence + deposit-paid sequence (needs scheduled sends — cron ticket). Note: API key was shared in chat during setup — rotate at leisure (Resend → API keys → delete + recreate, re-paste into Vercel + Supabase SMTP).
...
**S5 · Admin review queue → absorbed into The 120 CRM** *(dev)* — ✅ **Built 2026-07-13** — all 3 phases (P1 see the truth / P2 run the week / P3 close the loop) on branch `feat/crm`, PR open for review. 8 unit commits, 293 tests, 3 migrations applied to production (crm_core, crm_gtm, crm_library), staff seeded (peter@ + ethan@, passwords in gitignored local file). Plan: `docs/plans/2026-07-13-001-feat-the120-crm-plan.md` (status: completed). 11-reviewer autofix review done; residual work tracked in `.context/compound-engineering/todos/`. ⚠️ **Decision needed (P0 from review): production Supabase has email confirmations OFF (`mailer_autoconfirm: true`) — anyone can sign up with someone else's email, get linked to that email's lead family by the sync trigger, and force `consent_given = true` (forged CASL consent + lead hijack). Fix = enable email confirmations (changes signup UX) — Peter's call.**
