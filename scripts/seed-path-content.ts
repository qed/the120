...
import "../app/lib/fp/content/generated/program-2026-27";
import { getProgram, registeredVersions } from "../app/lib/fp/content/manifest";
import {
  buildProgramRows,
  buildUpsertSteps,
  checkSeed,
  ...
  type ProgramRows,
  type TableProbe,
} from "../app/lib/fp/content/seed-rows";

/** The version new students pin to. Its row carries is_current = true. */
const CURRENT_VERSION = "2026-27";

// The readiness-probe list is the SAME source of truth buildUpsertSteps writes
// against (a test pins them together), so a new table can never be added to one
 ...
 * defeating the precheck this script's docstring promises. `.limit(0)` returns
 ...
 * (app/lib/fp/content/seed-rows.ts): PGRST205 or a code-less transient error →
 ...
 * schema-cache lag right after a Management-API DDL apply and momentary edge
 * blips (retries the retryable cases); fails FAST and loud on an auth/permission
 * error so a credentials problem is never misreported as "apply the migration";
 * and aborts with a named error citing the migration if the tables genuinely
 * never appear.
 */
async function waitForTables(admin: SupabaseClient): Promise<void> {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    // Fail fast on a fatal (auth/permission) error — retrying cannot help and it
    // must not be dressed up as "the migration isn't applied".
    ...
    if (attempt === attempts) {
      throw new Error(
        `Path content tables still not reachable after ${attempts} attempts: ${missing.join(", ")}. ` +
          `Most likely the DDL migration is not applied — apply ` +
          `supabase/migrations/20260721120000_path_program_content.sql via the Management API ` +
          `(docs/solutions/integration-issues/supabase-cli-stale-db-password-management-api-workaround-2026-07-13.md) ` +
          `then re-run (a committed migration is not an applied migration). If the tables DO ` +
          `exist, this was a persistent transient/connectivity error — check the network and re-run.`
      );
    }
 ...
 * BUILT this run (checkSeed) — never a hard-coded total. Counts each table
 * SCOPED to the versions this run built, and — critically — checks the
 * is_current pin: exactly one row, and it is CURRENT_VERSION. Unit 6 reads that
 * pin to lock a student to a curriculum version, and ON CONFLICT DO NOTHING
 * cannot repair it once written wrong, so a mispin must fail here, not print
 * "Done."
 *
 * Counts are version-scoped (not whole-table) on purpose: old versions' rows are
 * permanent (D27), so an unfiltered count would false-fail if a future run's
 * import list ever omitted an already-seeded version. The is_current check stays
 * GLOBAL — its invariant is "exactly one true row across ALL versions".
 *
...
async function verify(admin: SupabaseClient, built: ProgramRows[]): Promise<void> {
  const builtVersionIds = built.map((r) => r.version.id);

  // The column keying a table to its version differs: path_program_versions is
  // keyed by `id`, the child tables by `program_version_id`. head:true is safe
  // HERE — it is a pure COUNT on tables waitForTables already proved present;
  // head:true is only unsafe as an existence check, which this is not.
  async function count(table: string, versionCol: string): Promise<number> {
    const { count: n, error } = await admin
      .from(table)
      .select("*", { count: "exact", head: true })
      .in(versionCol, builtVersionIds);
  ...
  const observed = {
    versions: await count("path_program_versions", "id"),
    phases: await count("path_phases", "program_version_id"),
    criteria: await count("path_criteria", "program_version_id"),
    tasks: await count("path_unit_tasks", "program_version_id"),
