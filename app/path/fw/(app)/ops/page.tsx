...
import FwCohortCreate from "@/app/path/fw/components/FwCohortCreate";
import FwWindowLabel from "@/app/path/fw/components/FwWindowLabel";
import { resolveFwStaffGate } from "@/app/path/lib/fw-auth";
import { listFwOpsCohorts, type FwBoardTokenStatus } from "@/app/path/lib/fw-ops-core";

/**
 * /path/fw/ops — every Founders Weekend, and the form that makes a new one
            ...
            <ul className="mt-5 space-y-3">
              {listed.cohorts.map((cohort) => {
                ...
                return (
                  <li key={cohort.id}>
                    <Link
                      href={`/path/fw/ops/cohort/${cohort.id}`}
