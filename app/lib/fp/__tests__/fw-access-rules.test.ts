...
describe("fwGuideInviteVerdict — Decision 12", () => {
  ...
  it("has NO session-email branch — the token is the credential", () => {
    // A guide invite is bound to its account and the claim replaces whatever
    // session the shared iPad was holding. The
...
describe("FW_GUIDE_INVITE_TTL_MS — its own constant (Decision 12)", () => {
  it("is 14 days", () => {
    ...
    // from issuing a batch to the doors opening. (It used to be asserted as
    // "not the co-parent invite's 7"; that feature and its constant were
    // removed in v3 plan Unit 10 and there is nothing left to differ from.)
