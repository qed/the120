 ...
 *
 * ⚠ NOT `revoked_reason`. Reserve that key for rows that actually carry
 * `revoked_at`: a photo withdrawal deliberately does NOT revoke the row, so
 * labelling it as a revocation reason would be a lie in the legal record.
 */
