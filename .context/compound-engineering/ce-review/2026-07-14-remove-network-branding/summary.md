...
|---|-----|------|---------|----------|------|-------|---------|
| 1 | P1 | supabase/migrations/20260713170000_crm_library.sql:91,110–115,135 | CRM copy-library seed rows (staff outbound email snippets) still say "TimeBack" / "Alpha Anywhere or GT Anywhere" and describe the old /parents copy — diverges from the rebranded site; brand names can reach families via staff emails | agent-native | high | manual → downstream-resolver | **Resolved 2026-07-14** — new migration `20260714210000_debrand_library_copy.sql` applied to production via Management API; verified 0 brand tokens across all 15 library_items and gtm_weeks actions. (Live titles carry ASCII hyphens, not the seed file's em-dashes — matched by LIKE prefix.) |
