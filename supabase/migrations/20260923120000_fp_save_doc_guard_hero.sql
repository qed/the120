-- First Profit game — fp_player_saves DOC GUARD v4: HERO CONFIG LWW. v3
-- (20260922120000_fp_save_doc_guard_story_fields.sql — APPLIED to production)
-- protects `businesses`, the per-idea monotonic maps/id, the `deletedIdeaIds`
-- tombstone union, the four monotonic story flags, and the `coverLook`/
-- `coverLookAt` LAST-WRITE-WINS pair. It knows NOTHING of the fpv03 U9a
-- `heroConfig`/`heroConfigAt` pair — the kid's chosen avatar, an EDITABLE
-- LATEST-INTENT value stamped at the client boundary, exactly like
-- `coverLook`/`coverLookAt` (see first-profit's src/data/storyBook.ts and
-- src/state/gameCore.ts). UNLIKE coverLook, this field has NO writer in
-- production yet — U9a is what first ships one — so this migration is landing
-- STRICTLY AHEAD of the risk window, not catching up to an already-live bug.
-- v4 replaces the guard function whole with v3's semantics carried forward
-- VERBATIM, plus one new graft.
--
-- ✅ VERSION — APPLIED TO PRODUCTION 2026-08-10 via the Management API
--   playbook (project deolvqnyvhhnavsifgxz), ledger version 20260923120000
--   recorded as a separate verified step AFTER the DDL succeeded. Live
--   confirmation: fp_player_saves_doc_guard prosrc grew 15316 -> 18737 bytes
--   and now contains the heroConfig graft + the v_new_hero_ok pairing vars;
--   trigger enabled, BEFORE UPDATE FOR EACH ROW.
--   POST-APPLY PROBES: ALL PASSED against the live trigger, run on a scratch
--   table with `request.jwt.claims` SET (the Management API session is
--   JWT-less and would otherwise take the exemption arm and pass vacuously —
--   the probe asserts session_exempt=false first). Passing: older-stamp-loses,
--   newer-stamp-wins, FULL-OMISSION-preserves-hero (the null-propagation
--   probe), v3 coverLook + flag still preserved under full omission,
--   config-without-stamp-loses (defensive pairing), docVersion-mismatch-
--   passes-through, equal-stamp-new-wins, absent-stays-absent.
--   PROBE-HARNESS GOTCHA, recorded so the next person does not lose an hour:
--   reset the scratch row between probes with DELETE+INSERT, never UPDATE. An
--   UPDATE reset is itself subject to this trigger, so restoring a seed whose
--   stamp is OLDER than what the previous probe left is correctly REFUSED —
--   every later probe then reads the wrong OLD and reports a false failure.
--
-- ⚠ ORIGINAL AUTHORING NOTE. Ledger-top confirmation: every prior
--   migration in this tree THROUGH 20260922120000_fp_save_doc_guard_story_
--   fields IS APPLIED TO PRODUCTION, so the v3 guard file must NOT be amended
--   in place — this change stacks as a new migration (the 20260909
--   convention). The slot below assumes the top of
--   supabase_migrations.schema_migrations is 20260922120000. The TRUE
...
--   gate); if the live top is not 20260922120000, RENAME this file to the
...
--
-- ⚠ DEPLOY ORDERING: this guard v4 is PROTECTIVE and must be LIVE IN PROD
--   BEFORE the first-profit build that writes `heroConfig` deploys. This is
--   the strict case: `heroConfig`/`heroConfigAt` have NO writer in production
--   today (U9a is the first client build to write them), so there is no
--   catch-up window the way there was for coverLook (which was already
--   shipping writes before its guard landed) — landing this guard first is
--   simply correct, not merely lower-risk. The original MIXED-BUILD ordering
--   from v1/v2/v3 still holds transitively, carried forward verbatim: this
--   guard must stay live BEFORE the first-profit build that writes
--   `businesses` / `doneByTask` / `doneAtByTask` (v1, applied), BEFORE the
--   first-profit build that ships DELETE_IDEA / writes `deletedIdeaIds` (v2,
--   applied THROUGH 20260910 and now live), and BEFORE the first-profit
--   builds that write the four v3 story flags / `coverLook` (v3, applied
--   THROUGH 20260921 and now live). Old clients against guard v4 are safe:
--   they omit `heroConfig`/`heroConfigAt` entirely (key-absent), and the
--   guard re-grafts OLD's values, so a stale write can never clear a flag,
--   clobber a newer cover choice, or clobber a newer hero choice.
--
...
--   1. All v3 probes (the v2 DELETION/OLD-BUILD/ACCIDENT probes, the
--      MONOTONIC-FLAG REGRESSION probe, and the COVER-LOOK LWW +
--      FULL-OMISSION probes — see the v3 header) still pass unchanged.
--   2. HERO-CONFIG LWW probe: seed OLD with `heroConfig: {"skin":"fox",
--      "outfit":"scout"}, heroConfigAt: 5000`, UPDATE with a NEW doc carrying
--      `heroConfig: {"skin":"owl","outfit":"chef"}, heroConfigAt: 1000` (an
--      OLDER stamp — e.g. a backgrounded tab flushing a stale in-memory
--      choice) → the response row's doc comes back with OLD's
--      `{"skin":"fox","outfit":"scout"}` / `5000` (the NEWER config
--      survives). Then UPDATE again with `heroConfig: {"skin":"wolf",
--      "outfit":"explorer"}, heroConfigAt: 9000` (a NEWER stamp) → the
--      response row adopts it verbatim.
--   3. HERO-CONFIG FULL-OMISSION probe — RUN THIS ONE, IT IS THE PROBE THAT
--      CATCHES THE NULL-PROPAGATION BUG. Seed OLD with `heroConfig:
--      {"skin":"fox","outfit":"scout"}, heroConfigAt: 5000`, UPDATE with a
--      NEW doc that OMITS BOTH keys entirely (the classic old-build write) →
--      the response row must STILL carry OLD's `heroConfig` object and
--      `heroConfigAt: 5000`. v3's coverLook graft shipped exactly this bug in
--      its FIRST DRAFT: `jsonb_typeof()` of an ABSENT key is SQL NULL, a
--      plain `<>` comparison on that NULL is NULL, the surrounding AND/OR
--      collapsed to NULL, and plpgsql's IF treated it as false — so the
--      first draft passed every OTHER probe (including the LWW-with-both-
--      sides-present probe above) while silently DROPPING the protected
--      field on the one write it exists to protect against. Never remove
--      this probe.
--   Then restore the row to its pre-probe state.
--
-- SEMANTICS v4 (mirrored byte-for-intent by the TS spec,
...
--   now parses THIS file). Everything from v3 is carried forward unchanged
...
--   never-raise posture, the service_role/JWT-less exemption); v4 ADDS,
--   inside the protected repair region, IMMEDIATELY AFTER the coverLook graft
--   and BEFORE the per-idea `ideas` gate (so the new graft applies even when
--   idea handling is skipped — exactly like the tombstone union and the
--   coverLook graft):
--
--   * GRAFT C — heroConfig / heroConfigAt, LAST-WRITE-WINS. Mirrors the
--     coverLook graft exactly, with `heroConfig` in place of `coverLook` (an
--     OBJECT rather than a string) and `heroConfigAt` in place of
--     `coverLookAt`. Mirrors the client's own persisted-doc merge with
--     local := NEW, server := OLD (the guard's repaired doc starts as NEW, so
--     NEW is the "local" / default-wins side and OLD only overrides it):
--       result.heroConfig   := NEW.heroConfig
--       result.heroConfigAt := NEW.heroConfigAt
--       if OLD.heroConfig is a jsonb OBJECT (jsonb_typeof(...) = 'object')
--          and OLD.heroConfigAt is a NUMBER
--          and ( NEW.heroConfig is not a jsonb OBJECT
--                or NEW.heroConfigAt is not a number
--                or OLD.heroConfigAt > NEW.heroConfigAt )
--       then result := OLD's pair (both heroConfig and heroConfigAt).
--     The pair is written back to the repaired doc ONLY when a config
--     survived (absent-stays-absent when NEITHER side carries an OBJECT
--     heroConfig); the stamp is written back only when it is a valid number.
...
--     own strict-greater tie rule (NEW wins ties).
--     OBJECT-SHAPE VALIDITY: the coverLook graft's "non-empty" test has no
--     meaning for an object — there is no such thing as an empty-but-invalid
--     object the way `""` is an invalid string — so the validity test here is
--     purely `jsonb_typeof(...) = 'object'`. This DELIBERATELY EXCLUDES a
--     jsonb ARRAY (jsonb_typeof returns 'array', not 'object', so an array
--     never satisfies the object test) and a jsonb STRING/NUMBER/BOOLEAN: a
--     hero config is a keyed record of choices (skin/outfit/etc), and neither
--     an array nor a scalar is an acceptable shape for it.
--     DEFENSIVE PAIRING: a stamped-but-configless side (a valid heroConfigAt
--     with no object heroConfig — a malformed/hand-edited doc) must never
--     null out a valid config on the other side. Requiring OLD.heroConfig to
--     be a present OBJECT before OLD can win blocks a configless OLD stamp
--     from clobbering NEW's config; NEW's own heroConfig/heroConfigAt are
--     carried forward verbatim (whatever shape NEW sent, valid or not)
--     whenever OLD does not win, so a configless-but-stamped NEW can never be
--     "fixed" into losing its (absent) config either — the guard never
--     invents shape NEW didn't send.
--
...
--
-- ⚠ NULL-SAFETY, THE WAY THIS IS WRITTEN IS LOAD-BEARING (carried forward
--   from v3's P0 review finding — READ THIS BEFORE TOUCHING GRAFT C).
--   `->` on a MISSING key yields SQL NULL (not jsonb 'null'), and
--   jsonb_typeof() is STRICT, so `jsonb_typeof(NEW.doc -> 'heroConfig')` is
--   NULL when NEW omits the key. A plain `<> 'object'` comparison on that
--   NULL is NULL, not TRUE; the enclosing OR/AND collapse to NULL; and
--   plpgsql's IF treats NULL as FALSE. Written that way the graft would NOT
--   fire in exactly the case it exists for — an old-build write that omits
--   the pair entirely — and since v_doc starts from NEW.doc, the child's hero
--   would be ERASED by the very guard meant to preserve it. v3's FIRST DRAFT
--   of the coverLook graft shipped exactly this bug and it was caught in
--   review before it reached production; the fix there is copied verbatim
--   here rather than reinvented: the presence tests use `is not distinct
--   from` (never NULL, regardless of key presence), and the stamp comparison
--   lives in its OWN NESTED branch (`elsif` under `if v_new_config_ok`,
--   itself under `if v_old_config_ok`) so the ::numeric casts are UNREACHABLE
--   unless both sides are already known numbers — PostgreSQL does not
--   promise left-to-right OR short-circuiting, and a raise inside the
--   protected region would discard the ENTIRE repair (businesses,
--   tombstones, the story flags, coverLook, per-idea grafts), not just this
--   one graft.
--
-- ACCEPTED FAIL-OPEN (JWT-less exemption arm) — carried from v1/v2/v3
...
--
-- ACCEPTED LOSS MODE (old-build idea fusion) — carried from v1/v2/v3
...
--
-- ACCEPTED BOUND (tombstone cap) — carried from v2/v3 verbatim: ids past the
...
--
-- ACCEPTED EDGE (size cap) — carried from v1/v2/v3 verbatim: grafting can
...
--   to stop — and is unreachable for any legitimately-sized save. Graft C
--   adds at most a handful of scalar/object keys, so this edge is not
--   meaningfully widened.
--
-- PROJECTION UNCHANGED: fp_public_site_content / fp_public_sites_project_save
--   (20260907..20260909) read doc->'ideas' only — heroConfig/heroConfigAt are
...
--   attributes and ACL, so everything v1/v2/v3 established is re-applied
...
begin
  ...
  -- and the story/cover/hero fields are additive-optional under docVersion
  -- 1); a differing version is a deliberate transition or a client that
  -- discarded a malformed/unknown-version doc — never resurrect into it.
