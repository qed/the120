...
import { phaseByKey, phaseColor, type PhaseKey } from '../system/phases';
...
/**
 * PhaseRow — one row of the HQ progress ledger. Criteria as five segments,
 * a task tally, and the phase's status (including the formal review banner).
 */
    ...
    <div
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="text-right">
          {status === 'sealed' &&
          <StatusChip state="verified" className="ml-auto" />
          }
          {status === 'review' &&
          ...
          {status === 'active' &&
          <span className="font-mono text-xs text-hq-ink-muted">
      ...
      <p className="mt-2 font-mono text-[11px] text-hq-ink-muted">
          Reviewer: {reviewer} · Countersign: Guide (pending)
        </p>
      }
      {status === 'sealed' && sealedDate &&
      <p className="mt-2 font-mono text-[11px] text-hq-ink-muted">Sealed {sealedDate}</p>
      }
    </div>);
