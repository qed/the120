 ...
 *                lastCompletionAt, lastCorroboratedCompletionAt,
 *                recencyClamped, hasCompletionsOutsideRequest}],
 *        businesses:[{id, ideaId, archived, doneByTask, doneAtByTask,
 *                     lastCompletionAt, lastCorroboratedCompletionAt,
 *                     recencyClamped, hasCompletionsOutsideRequest}]
 *      }],
 *      round1Payments?: {unit:"child", paidPurchases,
 *        complimentaryAccess, pending, unpaid, refundedPaid,
 *        revokedComplimentary}}
 * The optional payment summary is derived separately in
 * round-one-payment-rules.ts. It contains aggregate counts only and is OMITTED
 * when its provisional billing read is unavailable; absence never means zero.
 * The legacy `paid` bucket means active access (including comped/grandfathered)
 * and `refunded` includes revoked complimentary access; the staff client labels
 * those buckets as access status rather than misrepresenting them as revenue.
 * A CANONICAL sample of this body, produced by running the real `shapeProgress`
 * over a representative store, is committed in the first-profit repo at
 * src/screens/staff/__tests__/fixtures/progress-response.json and regenerated by
 * `npx tsx scripts/gen-fp-progress-fixture.ts` here. It exists because the wire
 * shape is DECLARED TWICE, in two repos, from the same reading — so a rename on
 * this side is otherwise a silent `undefined` at runtime with every test in both
 * suites green.
 ...
 * child surfaces, and never shared with the suggestions buckets).
 *
 * ── SIZING: what the Watchtower actually does ──
 * The original rationale sized these for "TWO authenticated GETs per /staff
 * visit plus at most one retry". That usage pattern no longer exists and had
 * not for a whole unit: the flow board is a CRITERION-AT-A-TIME view, and it
 * issues ONE request per criterion selection. A staff member sweeping the
 * curriculum spends up to 25 (every criterion), plus one per phase button (a
 * phase click selects that phase's first criterion), plus one per Refresh and
 * one per Retry. The client caches per criterion for the life of the page, so a
 * single sweep is ~30 requests and re-visiting a criterion is free — but a
 * sign-out, a tab reload or a Refresh-heavy session pays again.
 *
 * At the old 60/15min a staff member reading two phases with a few refreshes
 * crossed the budget mid-session, and the refusal is a 401 (see below), which
 * the SPA used to spell as "you are not staff". The numbers below allow ~10 full
 * curriculum sweeps in a window — comfortably past any human reading pattern —
 * while the per-IP aggregate stays the abuse bound: it is DOUBLE the per-user
 * budget, so one school's NAT fits two staff members at full tilt and a scripted
 * reader is still capped at 600 whole-cohort reads per 15 minutes. The limiter
 * remains this endpoint's cost fuse as well as its abuse fuse; the client polls
 * on no interval.
 *
 * ── Why `rate_limited` STAYS on the byte-identical 401 ──
 * Unit 1 moved CAPACITY refusals to the 400 class because the staff SPA read a
 * 401 as "not staff". The same move is NOT available here: capacity is checked
 * after both halves of the staff gate, while the limiter is recorded BEFORE any
 * DB I/O (house discipline — the gate itself must not be free to hammer). A
 * 400 there would answer any caller with a decodable JWT `sub`, handing a child
 * or an attacker an oracle that this endpoint exists and that they were
 * throttled — exactly what the one-voice 401 exists to deny. The client-side
 * half of the fix is the one that belongs to the client: a second 401 on a
 * session that has ALREADY been served by this API is not evidence about who the
 * caller is, and first-profit's StaffShell no longer treats it as a refusal.
 *
 * Both constants are PINNED by the unit test so any future tightening is a
 * deliberate edit rather than a drive-by.
 */
export const PROGRESS_RATE_LIMIT: RateLimitConfig = { windowMs: 15 * 60_000, limit: 300 };
export const PROGRESS_IP_RATE_LIMIT: RateLimitConfig = { windowMs: 15 * 60_000, limit: 600 };
...
export type WalkedIdea = ProgressCompletionMaps & {
  ...
  /**
   * The same number, but over stamps that ARE completions: the largest stamp
   * whose key also carries `true` in the matching boolean map, pre-filter, or
   * null when this record has no timestamped completion at all.
   *
   * This is the TIMING EVIDENCE behind `lastCompletionAt`, and it exists because
   * that field deliberately is not done-gated. Without it the client's only
   * corroboration for an out-of-window recency was
   * `hasCompletionsOutsideRequest` — a flag that carries no timing, so a child
   * who completed one task a year ago and quit corroborated every future stamp
   * they ever wrote, for every criterion, forever. A bare `doneAtByTask` key
   * with no `doneByTask` mints no completion anywhere in the system, yet it
   * moved `lastCompletionAt` to "just now" and, with that flag set, read as
   * ACTIVELY WORKING on a board whose entire job is noticing who has stopped.
   *
   * Read the pair, never one alone: when `lastCorroboratedCompletionAt` is older
   * than `lastCompletionAt`, the newest thing in this record is a stamp with no
   * completion behind it.
   *
   * Same clamp and same caps as `lastCompletionAt` — it is a selection of the
   * same stamps, not a second computation over the raw doc. It is bounded by the
   * boolean map's ENTRY cap, so a hostile doc can only make it OLDER (a
   * conservative direction: less credit, never more).
   */
...
export type WalkedBusiness = {
   ...
   */
  lastCompletionAt: number | null;
  /** As `WalkedIdea.lastCorroboratedCompletionAt`, over this business's own
   *  stable maps. The attack it closes lands hardest here: a business carrying
   *  one fresh bare stamp and no completions used to revive a 400-day-dead idea
   *  into `active` the moment it folded. */
  lastCorroboratedCompletionAt: number | null;
...
/**
 * The largest stamp that is backed by an actual completion — the timing evidence
 * behind `WalkedIdea.lastCorroboratedCompletionAt`.
 *
 * Both maps are already narrowed: `done` holds only `true`s, `stamps` holds only
 * finite, clamped, in-range numbers. Pairing them here rather than inside
 * `narrowTimestampMap` keeps that function ignorant of the boolean map it has no
 * business reading, and matches the client's own union rule — a timestamp
 * without its `done: true` is not a completion, anywhere in this system.
 */
function maxCompletionStamp(
  done: Readonly<Record<string, boolean>>,
  stamps: Readonly<Record<string, number>>
): number | null {
  let max: number | null = null;
  for (const [key, at] of Object.entries(stamps)) {
    if (done[key] !== true) continue;
...
function walkIdeas(rawIdeas: unknown, budget: WalkBudget): WalkedIdea[] {
  ...
  for (let index = 0; index < rawIdeas.length; index++) {
    ...
    const done = narrowBooleanMap(raw.done, budget);
    const doneByTask = narrowBooleanMap(raw.doneByTask, budget);
    out.push({
      index,
      ...
      lastCorroboratedCompletionAt: laterOf(
        maxCompletionStamp(done, doneAt.map),
        maxCompletionStamp(doneByTask, doneAtByTask.map)
      ),
 ...
 *
 * EVERY DROP FLAGS `truncated`. A business has no `index`, so — unlike
 * `walkIdeas`, which emits an index-preserving placeholder — a dropped entry
 * leaves NO trace on the wire, and the client's only loss detector reads
 * `idea.index`. This was the one hole in the feature's "absence is a number,
 * never a filter" rule: three paths (non-object, missing/empty/over-long id,
 * duplicate id) deleted a business record with the child's whole Phase 4-5
 * progress on it while the row reported nothing abnormal at all. A placeholder
 * is not available (there is no id to give it, and a minted one would collide
 * with a real record), so the flag is: `truncated` already means "this doc is
 * abnormal — do not read a normal-looking row as a promise", and the client
 * already surfaces it as a counted caveat.
 */
function walkBusinesses(rawBusinesses: unknown, budget: WalkBudget): WalkedBusiness[] {
  ...
  for (const raw of rawBusinesses) {
    ...
    // `narrowAuthoredId` already flags an OVER-LONG id; absent, non-string and
    // empty ones reach here as a bare null and must flag too.
    ...
    out.push({
      ...
      lastCorroboratedCompletionAt: maxCompletionStamp(doneByTask, doneAtByTask.map),
