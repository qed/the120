 ...
 * band variant, and a quiet status chip. Plain, confident, no cheerleading.
 */
    ...
    <motion.article
      ...
      <header className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-mono text-xs text-hq-ink-muted">{task.id}</span>
            {task.liveMoment &&
            <span className="inline-flex items-center gap-1 rounded-full bg-hq-ink px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white">
                <RadioIcon className="h-3 w-3" /> Live moment
              </span>
            }
          </div>
      ...
      <div className="mt-4">
          ...
          onClick={onOpen}>
            ...
            {task.state === 'not_yet' ? 'Resubmit' : task.state === 'verified' ? 'View evidence' : 'Open task'}
          </Button>
        </div>
      }
    </motion.article>);
