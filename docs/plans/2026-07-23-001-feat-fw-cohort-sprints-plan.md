...
**PROPOSED — ALL THREE DECIDED (user, binding):** **PROPOSED-1 ACCEPTED** (2026-07-23; the cross-cohort matching module ships; its first seam is Unit 4 quick-create, then Units 5b and 7). **PROPOSED-3 REJECTED** (2026-07-23; no paper-weekend backfill: no retroactive consent notice, no transcription owner, no opt-out handling — Unit 7 ships the Boston roster import only, and every `*(PROPOSED-3)*` sub-item is struck). **PROPOSED-2 ACCEPTED** (2026-07-24, ahead of its ~Aug 8 gate; Unit 6 renders the persistent First Dollar counter beside the XP hero — one board component + one aggregate, semantics fixed by Decisions 5 and 16, undo-safe because recomputed and with no freshness term; every `*(PROPOSED-2)*` sub-item is now a committed requirement, not a conditional). Historical note: PROPOSED-1/-3 were originally gated by **~Aug 1** and PROPOSED-2 by **~Aug 8** (before Unit 6 builds the board); all three are now settled. Unit 9 verifies seam states; it never hosts the decisions.
...
- [x] **Unit 1: FW schema migration, email builder, FW materializer, no-auth-mail mechanism** — landed 2026-07-23. Migration `20260728120000` **APPLIED to production 2026-07-23** (all nine post-apply checks green; version recorded); both email probes still outstanding. Implementation-time resolutions: idempotency shape = `action_id` + `client_id` columns on `path_task_events` with a partial unique index (over a dedupe side-table); the normalized-name lookup ships as a stored `normalized_name` column + partial index rather than an expression index (the NFKC fold is not an immutable SQL function); FW profiles keep `family_id` NOT NULL via a private single-student `path_families` row rather than loosening a column ~12 Path reads assume.
...
**Dependencies:** None. **Timing note (SUPERSEDED 2026-07-23):** this unit originally held the production apply until after a ~Jul 28 Chicago-rehearsal checkpoint. **Chicago was cancelled and the hold is retired repo-wide — every migration now applies as soon as it is authored**, via the Management API playbook. The rule the hold protected still stands and always did: a schema revision after apply is a new production migration, not an edit. (Week-1 side task rides along: the two email probes — live `createUser` on the real domain confirming zero mail; probe mail to a nonexistent `*.fw@` address confirming bounce/no-catch-all.)
...
- [x] **Unit 2: FW authz — admin bridge, guide door, guide provisioning** — landed 2026-07-23. Migration `20260729120000_fw_guide_invites.sql` **APPLIED to production 2026-07-23** (RLS on, zero policies, version recorded). Implementation-time resolutions: (a) guide invites needed a table Unit 1's seven DDL groups did not include, so `path_fw_guide_invites` ships as its own migration file (one row per guide account, rotated in place — that is what makes "a re-issue kills the old hash" structural rather than a discipline) rather than an eighth group in a merged, already-reviewed file; (b) the unit added two modules beyond the plan's file list — `fw-guide-core.ts` (the plain db-taking core, so the sequencing/compensation is testable per the house rule that impure shells get tested too) and `fw-auth.ts` (the server-only session shell, which deliberately permits ZERO grants because bridge-resolved staff hold none — `requirePathUser` would 404 exactly the people who run the event); (c) `resolveFwActor` refuses `kind='path'` cohorts for GRANT-holders too, not only for the bridge as the plan's sentence read — a `guide`/`cohort` grant on a Path cohort is the Path's D25 reviewer grant, and letting it open the cascade-free FW write path would hand every Path cohort guide a direct editor for their students' records; (d) issuance has two modes, `ensure` (provisioning) and `reissue` (staff recovery) — review found that issuing unconditionally after every successful provision turned the plan's own "a guide works Boston and Hamptons" flow into a credential rotation that un-marked a working guide as unclaimed and mailed them a live password-setting link; (e) `/path/fw/board/` is carved out as unguarded now per the plan, but no route exists until Unit 6 — **Unit 6 must validate the token per request; the proxy will never gate that subtree**.
...
- [x] **Unit 3: `fw_move_task` RPC + check-in write path** — landed 2026-07-23. Migration `20260730120000_fw_move_task.sql` **APPLIED to production** (all post-apply checks green, `move_path_task` verified untouched, version recorded), then exercised end-to-end against the live schema in a self-rolling-back `DO` block: all twelve decision-table scenarios returned the expected outcome, 6 events, 0 reviews, 0 unlock events, zero residue. Implementation-time resolutions: (a) the migration is `20260730120000`, not the plan's `20260801TBD` — it applies today, so it sorts immediately after Unit 2's; (b) **the event's `transition` column carries the FW action name** (`checkmark`/`not_yet`/`undo`), not a Path transition name — `path_fw_replay_rejects.action`'s CHECK already speaks exactly that vocabulary, and it keeps the two write paths distinguishable in the one shared log; (c) the RPC takes a `SELECT … FOR UPDATE` *above* the predicated UPDATE — not as a substitute for the WHERE guard (which stays, and is parity-asserted) but because the event needs a truthful `from_state` and `UPDATE … RETURNING` yields new values, not old; holding the lock also makes the zero-row classification strictly stronger than the plan's "re-read after"; (d) the RPC re-asserts the cohort stamp itself (membership ∩ `kind='fw'`, outcome `cohort_invalid`) as its one defense-in-depth check, on `move_path_task`'s stated posture — a forged stamp is not a failed write but a permanent lie in an append-only log; (e) `decided_at` is cleared on undo (the plan named only the author) — a `locked` row is not a decision; (f) a re-attempt leaves the row **completely** untouched including its author, so undo rights stay with whoever made the decision rather than transferring to whoever tapped last; (g) the unit added `fw-checkin-core.ts` beyond the plan's file list — the plain db-taking core carrying the batch orchestration, so the composition is testable per the house rule Unit 2's P1 established, leaving `actions/fw-checkin.ts` with nothing decision-bearing in it; (h) `FwEcho.state` is nullable, because `missing` is the one outcome with no row to describe — every other outcome with a null state fails closed rather than reporting a success on a row nobody can name. **Review-driven changes (11 reviewers; 4 findings applied, 2 accepted-and-documented):**
- **P1, adversarial — the exactly-once key was globally unique, not scoped to the tap it names.** Unit 1 shipped `client_id` with a partial unique index on `(client_id)` alone while its own comment defined the value as "per (student, task, tap)". Two students sharing a value meant the second student's check-in was classified `replayed` — a success-shaped outcome — and silently LOST; the concurrent variant moved a row's state while `ON CONFLICT` dropped its event, splitting state from audit. Fixed by `20260731120000_fw_client_id_scoped.sql`: the index and both `ON CONFLICT` targets become `(student_id, task_id, client_id)` and the RPC's replay probe filters on all three. Applied to production and proven against it (two students sharing one `client_id` both apply; a genuine same-student-same-task replay still no-ops; every applied row has an event).
- **P1, testing — two parity assertions could not fail.** `toContain("cohort_id")` was satisfied by the INSERT's column list regardless of what VALUES bound, so nulling the cohort stamp shipped green; and the grant/revoke block scanned raw source, so commenting out `revoke … from anon, authenticated` on a SECURITY DEFINER function also shipped green. Both now assert against comment-stripped SQL and the VALUES row specifically.
- **P1, reliability — no timeout anywhere.** A stalled call over venue wifi was bounded only by the platform default. Added `FW_CALL_TIMEOUT_MS` (8 s) with a discriminated race wrapper, plus try/catch so a thrown error cannot unwind the batch.
- **P2, performance — the batch loop's stated rationale was wrong.** It ran sequentially "because each call takes a row lock", but `planFwBatch` de-duplicates by student, so the targets are always distinct rows sharing no lock. Now concurrent, saving ~600 ms of the ~5 s tap-to-board budget on a 3-student tap.
- **P2, maintainability — `isFwAction`/`fwActionTarget` shipped with only their own tests as callers**, the exact pattern the Unit 2 learning warns about. Both now have production callers; the speculative `TASK_STATES` re-export was deleted.
- **ACCEPTED, correctness P1 — Decision 2's "exactly one event" for checkmark × undo holds for one ordering, not both.** If the undo commits first, the checkmark acts on `locked` — its primary legal source — and legitimately re-applies. This cannot be narrowed away without breaking FW-D5 (every FW row starts `locked`), and the `expected_from` CAS that would close it was deliberately dropped from this RPC's signature. **Decision 2's sentence is hereby qualified**: the guaranteed invariant is that an event exists iff the row moved and every `from_state` is the state its writer actually saw — no ordering yields a lie, a lost update, or a duplicate event for one decision.
- **CARRIED TO UNIT 4 — online taps carry no `client_id`.** `checkmark`/`undo` are idempotent by state, so an ambiguous-failure retry is safe; `not_yet` is not, because a repeat tap is *defined* to append a re-attempt event (the FW-D4 struggle signal). Unit 4 must mint a client id per tap ONLINE as well as offline; the whole path already exists. Pinned by test so the gap is visible in the suite.

Parity assertions are **mutation-tested**, not assumed: removing the WHERE guard reddens 3 tests, narrowing the author stamp 2, flipping the guard's `and` to `or` 2, un-scoping the client_id probe 3, nulling the cohort stamp 1, and commenting out the anon/authenticated revoke 1.
...
- [x] **Unit 4: Guide navigation, roster, quick-create** — landed 2026-07-23. **No migration** (as expected). Verified live against production on an iPad-sized viewport with a seeded 30-student rehearsal cohort — the full find→tap loop is four taps and ~1.4 s of server time (roster 456 ms · student 430 ms · task 449 ms · check-in action 450–558 ms, dev-mode warm, against production Supabase), so the minute is spent by the guide, not the system. Implementation-time resolutions:
- (a) **Route shape carries the cohort in the URL** — `/path/fw/cohort/[cohortId]/student/[studentId]/task/[taskId]`, not the plan's `(app)/student/[studentId]`. Decision 3 says the cohort stamp is always carried, never inferred; a cohort held only in device storage would be inferred by every page and unverifiable by any of them. In the path it is re-resolved against authoritative rows per request, and a deep link is complete. The per-cohort shell (`cohort/[cohortId]/layout.tsx`) is therefore the first layout that knows the active weekend, so the chrome lives there and `(app)/layout.tsx` is canvas-only.
- (b) **Two rules modules, not one.** `fw-match-rules.ts` per the plan (PROPOSED-1), plus `fw-nav-rules.ts` for search / duplicate flagging / resume derivation / tree shaping / the batch cap — navigation has no SQL counterpart and folding it into `fw-rules.ts` would grow the write-path module (imported by Units 5b/6/7/8) with UI shaping nothing downstream reads.
- (c) **The Unit-3 carry-forward is closed and proven**: `createFwClientIdLedger` mints one exactly-once key per (task, student, action), REUSES it across every retry of that tap, and releases it only on a settled outcome (`isFwResultSettled` — everything except `failed:unavailable`). Verified in production: four taps on one row produced four events with four distinct `client_id`s, including the `not_yet → not_yet` re-attempt (`from_state === to_state`).
- (d) **`fw-loader.ts` is a plain db-taking core**, not `server-only`, so its composition is testable with the fake-client harness (the house rule Units 2 and 3 each paid a P1 for). `loadFwStudentDrilldown` carries the READ-side of Decision 3 — a member check before the profile read — and 404s a cross-cohort URL edit. Verified live: 404 in 538 ms.
- (e) **PostgREST's 1000-row cap is real and was silently truncating a read.** Found by seeding: 3,750 progress rows came back as exactly 1,000 with no error. At ninety students a weekend's *decided* rows alone clear it, so two thirds of the roster would have carried a silently under-reported resume chip, worsening as the weekend went on and invisible to any fixture-sized test. Every list read in `fw-loader.ts` now pages via one `fetchAllRows` helper that refuses (never truncates) at its bound.
- (f) **Quick-create adds a post-write leg VERIFICATION** beyond `provisionFwStudent`'s sequencing: its materialization step is an `upsert … ignoreDuplicates` that reports `created: 0` on a resume whether the student has 125 rows or 3. `verifyFwStudentLegs` observes account → membership → materialization as a SET comparison against the pinned version's task ids, and `runFwQuickCreate` refuses to report success (handing back `retryProfileId`) until all three hold.
- (g) **A same-cohort PROPOSED-1 match ROUTES, it does not re-provision.** The confirm card offers "Open" per match with its band; `existingProfileId` is reserved for retry-in-place after a failed leg. This keeps `provisionFwStudent`'s identity gates from ever being asked to merge two children who share a name.
- (h) `fwMatchKey` is exported so the DB lookup and the matcher key on ONE function, and it refuses a name missing EITHER part — agreement with `buildFwLocalBase`, which `provisionFwStudent` would refuse the submission by moments later.
- (i) Two seed scripts ship with the unit: `npm run seed:fw-cohort` (idempotent, `--dry-run`, dedupes through `fw-match-rules` itself) and `npm run seed:fw-guide`. Rehearsal surnames carry a `-Rehearsal` suffix so no real student's name-derived address is ever burned — FW-D2 makes that address permanent and Decision 10's ledger makes a freed one unrecoverable.
- (j) Device preferences (active cohort, reading-rule dismissal) read through `useSyncExternalStore` in `fw-device.ts` rather than `useEffect`+`setState` — React 19's `set-state-in-effect` rule flags the latter, and two components read these keys while a third writes one, so a per-component effect would give each its own stale copy.
- **Live walkthrough fix:** the batch picker's "3 at a time is the maximum" note survived a successful action, reading as a refusal of the tap that had just succeeded. Cleared with the selection.
- **Production rehearsal data (removable):** cohorts `rehearsal-unit4` (30 students, 3,750 locked rows) and `rehearsal-unit4-second` (empty, exercises the multi-cohort switcher); guide `rehearsal.guide@the120.invalid`. Two students hold a 1.2.4 checkmark and one holds a not-yet plus a re-attempt. Unit 5b's anonymize path is what retires them.
...
- [x] **Unit 5: Staff FW ops — core (Boston-critical)** — landed 2026-07-23. **THREE migrations**, all APPLIED to production and recorded (`20260801120000_fw_ops_audit`, `20260801130000_fw_cohort_time_zone`, `20260801140000_fw_grant_scope_idx`); the live CHECK constraint, both immutability triggers and the index plan were verified against production, not asserted. Verified end-to-end against production via `npm run fw`: cohort create (09:00 ET → 13:00Z), mint, re-mint revoking the prior with `revoked_by` set, guide add, an idempotent re-add writing NO second audit row, grant revoke, and a second token-revoke correctly refusing. A live GUIDE session 404s on `/path/fw/ops` and sees no Ops link — the negative case, verified in a browser. Implementation-time resolutions:
- (a) **`path_fw_board_tokens` needed nothing, but the plan's own Scope Boundaries sentence was false.** "Cohort/token metadata mutations are already actor-attributed by their own rows" held for neither: `path_cohorts` had no `created_by` and a token recorded who MINTED it but not who REVOKED it, so an explicit revoke — the action that takes a projector down and leaves it down — named nobody. Both columns added, nullable.
- (b) **A second migration for `path_cohorts.time_zone`.** Decision 4 stores the window as `timestamptz`, which is an instant and carries no zone — right for every comparison, wrong for the one thing a human does with it: read it back and check. Without the zone the ops surface can only render UTC (correct, unreadable) or the VIEWER's zone (readable, wrong for anyone outside the host city), and `ends_at` is the value the plan flags as able to silently expire a board. Deliberately NO CHECK constraint — one enforcement point in `fw-ops-rules.ts`, per the CRM allowlist-drift learning applied in reverse.
- (c) **A third migration for `path_role_grants (scope_type, scope_id, role)`** (performance review). Unit 5 introduced the first queries in the repo that filter grants by SCOPE rather than by user; the only existing index leads with `user_id`, so both ops reads sequentially scanned a table that grows with the whole tutoring program. Verified with `enable_seqscan=off` that the index serves all three predicates as Index Cond.
- (d) **Two rules modules, not one.** `fw-board-rules.ts` per the plan (token mint/verify/expiry, and Unit 6 grows it into the board read model); `fw-ops-rules.ts` for the cohort record staff maintain — the timezone-explicit window conversion, the slug, and the audit vocabulary. They meet at exactly one value (`ends_at`) and it crosses as a string.
- (e) **The grant audit lives INSIDE `provisionFwGuide`**, the repo's only writer of `guide` grants, so none of its three call sites can bypass it — and it fires only when the upsert's `ON CONFLICT DO NOTHING` actually inserted, so the idempotent re-run does not invent an event. Proven against production: the second `guide-add` wrote no second row.
- (f) `fetchAllRows` moved from `fw-loader.ts` into `fw-call.ts` (the move that file's header already records for `withFwTimeout`) so the ops reads share one definition of the 1000-row bound rather than a second copy.
- (g) **`pickFwCurrentBoardToken` replaced an `order by created_at desc limit 1`.** Two token rows can share a `created_at`, and on a tie the database may return either — flipping a cohort's ops status between "live" and "revoked" on consecutive loads with nothing changed. The rule (live if there is one, else most-recently-revoked) is now pure and tested.
- (h) **Ops lives at `/path/fw/ops`, NOT under `cohort/[cohortId]/`** — that shell is the guide's working header and is scoped to one weekend, while ops spans cohorts and half of it (creating a weekend) has no cohort at all.
- **Review-driven changes (16 reviewers; 9 findings applied, 2 accepted-and-documented):**
- **P1, reliability + correctness — every ops WRITE bypassed the timeout guard** that `fw-call.ts` claims covers writes and that Unit 3's `fw_move_task` already followed. A stalled mint left staff on "Minting…" indefinitely with none of the compensation branches reached. Added `fwWrite` (`fwRead`'s twin over one shared body) and routed every mutation through it.
- **P1, adversarial — a guide grant could be added or revoked with NO audit row, permanently and invisibly.** The audit write was gated on the mutation's REPORTED success, and `fwWrite`'s own contract is that a timed-out write may still have landed. Worse, the RETRY conceals it: the add path's correct `ON CONFLICT DO NOTHING` reports "inserted nothing", so `audited: true` *vacuously*, and the UI shows an ordinary success. Both paths now POST-WRITE VERIFY and audit what actually happened, marked `recoveredFromReportedFailure`.
- **P1, adversarial — revoke killed "whatever is live", not the token staff saw.** A re-mint landing in between meant one staffer's revoke took down another's seconds-old token, possibly already on the projector, with both getting success. Revoke now CASes on `expectedTokenId` and reports `stale_view`.
- **P1, reliability — the guide-list Admin API fallback bypassed `fwRead`**, so a throw escaped `Promise.all` and crashed the whole ops page rather than degrading to its tri-state branch.
- **P1, kieran-typescript — `revokeBoardTokenAction`'s inline switch had NO exhaustiveness tripwire.** The other three passed TS2366 only INCIDENTALLY, because code after the block dereferenced a success-only property; this one's success arm is a bare `{ ok: true }`, so a reason added later (Unit 5b) would have fallen through and reported a FAILED revoke as success. All four extracted to helpers with declared return types.
- **P1, frontend-races — a just-minted, uncopyable token could vanish**: the page rendered `FwBoardToken` only inside the `token.ok` branch, so a transient read failure on the post-mint refresh unmounted it. Now rendered unconditionally with the read-failure state inside; both components rebuilt on single state machines.
- **P2 — `recordFwOpsAudit` moved to its own `fw-audit-core.ts`** so the guide door no longer pulls the entire ops core into its module graph.
- **P2, cli-readiness + agent-native — the CLI could create weekends and revoke access but could not get a credential into a guide's hands**, which is exactly Decision 12's Friday-morning recovery. Added `guide-reissue`, plus per-command flag scoping (a global allowlist let a misplaced flag silently no-op), usage listing every command's flags, and `--force` before replacing a live board (the UI confirms; the CLI silently did it).
- **ACCEPTED — two staff minting concurrently both get `ok:true`, but only the later token stays live.** The partial unique index guarantees at most one live token, never zero; what it cannot do is tell the first staffer their copied URL just died. Stated as an ops-briefing line rather than mechanised, alongside the existing "a mid-event re-mint kills the projector" line.
- **ACCEPTED — a cohort with audit rows can never be deleted.** `path_fw_ops_audit.cohort_id` is RESTRICT and the rows are immutable by trigger, so a mistyped cohort is permanent and visible in the ops list forever. Unit 5b should consider an archive flag; the verification cohort `unit5-verify` is the first instance.
- **Production residue (removable):** cohort `unit5-verify` (`b3e0738f-8a65-429a-8600-982d1b34c17f`) with 2 revoked tokens and 2 audit rows, and guide `unit5.verify.guide@the120.invalid` (grant already revoked). The audit rows are what make the cohort undeletable — see above.
...
- [x] **Unit 5b: Staff FW ops — completeness** — landed 2026-07-24. **ONE migration** (`20260801150000_fw_anonymize_action.sql`) **APPLIED to production and recorded** (constraint reconfirmed against the live catalog first; the action CHECK now reads exactly `('guide_grant_added','guide_grant_revoked','student_anonymized')`, the two immutability triggers untouched, verified against production not asserted). The anonymize CORE was proven end-to-end against production via `npm run fw` on a rehearsal-unit4 student (the plan's named anonymize target): name tombstoned (`Removed`/`student`, band + XOR intact), **4 task events RETAINED**, freed local part `maya.chen-rehearsal` written to the released-alias ledger, auth email renamed to `removed-<id>.fw@` (inside the no-auth-mail namespace, `email_confirm: true` so no change-confirmation flow), and a `student_anonymized` audit row written; a same-named sibling on the suffixed `maya.chen-rehearsal2` was left untouched, the anonymized student vanished from the name-match lookup, an idempotent re-run returned `alreadyAnonymized: true`, and a wrong typed name returned `confirm_mismatch`. Implementation-time resolutions:
- (a) **The alias is recorded BEFORE the rename, not after** (a deviation from Decision 10's prose ordering, taken for crash-safety): once the email is renamed to `removed-<id>.fw@`, the original — possibly suffixed — local part is UNRECOVERABLE, so a rename-then-alias order that died between the two would free `maya.chen` with no ledger row and let the next Maya Chen be minted onto a channel the first family still holds. Recording the alias first makes the freed-address ledger consistent at every intermediate state; both writes still happen. The rename's error branch POST-WRITE VERIFIES (a landed-but-timed-out rename still gets its audit row), and the already-anonymized path re-attempts a missing audit — so the liability record is self-healing across every partial-failure shape (the harness's `applyAnyway` flag exercises the landed-but-reported-failed rename).
- (b) **The typed confirm is verified SERVER-SIDE**, not just in the browser: the core compares the typed name to the stored name through `buildNormalizedFwName` (`fwAnonymizeConfirmMatches`), and the sequence is RESUMABLE — a run that tombstoned the name but died before the rename detects the tombstone and skips the confirm (the name is gone) rather than deadlocking.
- (c) **No new subject column** — the Unit 5 migration's speculative "adds its own subject column" comment was decided against: an FW student has a `user_id`, which is exactly what `subject_user_id` references. The one schema change was the CHECK widening.
- (d) **Reject-resolve and link membership write NO audit row** (Scope Boundaries: only anonymize and guide-grant changes are liability-audited); resolve is scoped to the cohort by predicate so a forged id from another weekend cannot be closed, and link adds only a membership (progress is per-student, so a returner's record arrives filled).
- (e) **PROPOSED-1 staff resolution** (`loadFwMatchResolution`) shows the full cross-cohort detail the guide's minimal signal withheld (band + every cohort membership by slug); an anonymized student — whose `normalized_name` the sequence NULLs — never surfaces, which is name-unfindability working.
- (f) **Agent-native parity**: `npm run fw` gains `students`/`rejects`/`reject-resolve`/`anonymize`/`match`/`link`, driving the SAME cores the surface calls, with the same typed-confirm verification.
- **Not verified**: the SURFACE render (the three new ops sections) needs a staff/bridge session to drive — the rehearsal guide resolves via grant, not bridge, so it correctly sees none of ops. The core is proven; the surface is built to the established patterns, typechecks clean, and awaits a bridge login at the dry run. **Production residue (removable):** rehearsal-unit4 student `9e88d742…` (Maya Chen-Rehearsal) is now anonymized — the plan's named retire target.
- **Review-driven changes (13 personas + a focused re-review; 1 P0, 1 P1, several P2s applied):**
  - **P0 (adversarial re-review) — an anonymized student was still checkin-able.** Anonymize keeps the `path_cohort_members` row by design, and the earlier fixes closed the re-link and roster-LIST doors but NOT the write path: a guide with a task page rendered before the anonymize (stale tab / race) could still tap → `runFwCheckIn` → `fw_move_task` (which only checks membership + `kind='fw'`) and write real events for the retired identity. Closed at the sole write-path choke point — `runFwCheckIn` now reads tombstone status concurrently with membership and excludes anonymized students (a non-member skip, fail-closed on a read error) — plus a `loadFwStudentDrilldown` 404. Verified live: the anonymized rehearsal student's check-in returns `skipped`, event count unchanged. A redundant guard inside the parity-frozen `fw_move_task` RPC was deferred (no reachable bypass; would duplicate the sentinel into SQL) and noted for Unit 8's drain (also routes through `runFwCheckIn`).
  - **P1 (adversarial) — `linkFwStudentToCohort` could resurrect an anonymized identity.** A stale cross-cohort match entry could link a since-anonymized student into a new weekend (the tombstone keeps band + child_id, so the FW-shape gate passed). Now refuses a tombstoned profile (`student_anonymized`); `loadFwProfiles` also filters anonymized students out of every guide roster / batch picker so a returner anonymized from one weekend never ghosts on another.
  - **P2 (correctness + adversarial) — concurrent anonymize double audit.** Two staff anonymizing one student both wrote a `student_anonymized` audit row. New partial UNIQUE index (`path_fw_ops_audit_one_anonymize_idx`, migration `20260801160000`, applied + recorded) + a probe-then-insert writer that treats the unique violation as success + a `.limit(1)` multiplicity-tolerant probe. Migration `20260801160000` also adds a student-scoped partial index for the cross-cohort open-reject reads (perf).
  - **P2 (correctness) — the roster hid the resume affordance for a partial removal.** `listFwOpsStudents` now derives `anonymizeComplete` from the audit row; the surface offers "finish removing" for the tombstoned-but-not-audited state. Plus reliability (account read-error split), performance (confirm gate before the account fetch), maintainability (reuse `fwAnonymizeConfirmMatches` in the component), CLI (`anonymize --force`), a dedicated no-auth-mail lane forcing every admin `updateUserById` email change onto a reviewed allowlist, migration-parity hardening (table-anchored CHECK scan, ALTER-inside-IF, drop/disable-trigger guards), and the fake-db harness now honoring `applyAnyway` on insert/update.
- **Carried to Unit 9:** the plan-review gap that a concluded cohort holding audit rows cannot be deleted (RESTRICT + immutable audit) is untouched here — an archive flag is Unit 9's to consider, not forced into 5b.
...
- [x] **Unit 6: The projected board** — landed 2026-07-24. **ONE migration** (`20260802120000_fw_board_events_cohort_id_idx.sql`) **APPLIED to production and recorded** (index verified as `btree (cohort_id, id) where cohort_id is not null` against the live catalog, not asserted — a review-surfaced perf need, exactly the "a board read that needs an index is a new migration" path the unit anticipated). The read model, loader, token route (page + feed), and polling board were verified live against production `rehearsal-unit4` via a minted token AND `npm run fw -- board`: 29 grid rows (30 − 1 anonymized), weekend XP 4, First Dollar counter 3 vs. 2 name-bearing fresh celebrations (honest-when-stale on real data), feed `private, no-store` + `noindex`, the page carries ZERO student names, and a garbage/expired/revoked token 404s with no cohort-existence leak. Implementation-time resolutions:
- (a) **The anonymized-member board decision (the one the unit flagged to decide, not ignore):** an anonymized student (Unit 5b tombstone, `isFwTombstoneName`) is EXCLUDED from the grid, the ticker, and celebration naming, but their RETAINED events still count in every weekend AGGREGATE — XP, the First Dollar counter, checkmark/not-yet rollups (Decision 10: task ids are not PII). The read model splits `memberIds` (all, for aggregates) from `nameBearingIds` (for display), mirroring `loadFwProfiles`' guide-roster filter without inheriting it (the board is a separate read model, `fw-board-loader.ts`). Proven live: the live rehearsal anonymized student's retained 1.2.4 counts toward the counter (3) but is one of the two it does NOT name (celebrations 2).
- (b) **A force-dynamic page CANNOT be served `no-store`** — Next fixes its `Cache-Control` to `no-cache, must-revalidate`, and neither `next.config` `headers()` nor the proxy overrides it (verified empirically). Rather than argue a minor's name is safe in a `no-cache` frame, the PAGE renders only a **PII-free shell** (`loadFwBoardShell` — cohort slug + column skeleton, no names) and EVERY student name flows through the `private, no-store` `/feed` the client polls. `X-Robots-Tag: noindex` from `next.config` DOES apply to the page (Next overrides only Cache-Control). Compounded: `docs/solutions/best-practices/force-dynamic-app-router-page-cannot-be-no-store-…`.
- (c) **The poll endpoint is a Route Handler inside the UNGUARDED token subtree** (`board/[token]/feed/route.ts`), so the unguarded prefix + the per-request hash check provably cover the payload, not just the page (Open Questions resolution). `resolveFwBoardToken` (shared by page and feed) hashes → looks up → runs `fwBoardTokenVerdict`, collapsing garbage/expired/revoked/unreadable/read-error into ONE `{ok:false}` → one bare 404, fail-closed on a read error. `hashFwBoardToken` relocated to its own tiny `fw-board-token.ts` (re-exported from `fw-ops-core`) so the unauthenticated route need not import the whole ops graph.
- (d) **Grid `columns` ride the feed and the client resyncs them each poll.** A board opened BEFORE check-in (the natural setup order → empty cohort → empty column skeleton) would otherwise render a columnless grid for the whole event; now it fills the moment the first member is checked in (adversarial review).
- (e) **The event fold tiebreaks `(atMs, capturedAtMs, id)`, not `(atMs, id)`.** `at` is ms-truncated and `id` is a random uuid, so a same-millisecond checkmark/undo pair (Unit 8's drain) would resolve by coin flip and could STABLY report the wrong current state; capture (tap) order — which a single actor's drain queue preserves — breaks the tie (adversarial review).
- (f) **No-per-student-XP is structural** (FW-D13): the grid row type/runtime carries exactly `{studentId, displayName, cells}` — a test pins the EXACT key set. **PROPOSED-2** (accepted 2026-07-24) shipped as one aggregate (distinct students with a non-undone cohort-stamped 1.2.4 verify, no freshness term) + one persistent co-hero beside the XP number.
- **Review-driven changes (12 reviewers; every actionable finding applied, no P0):** scrubbed a NUL byte in the read model; on a **404 the client CLEARS the frame** so revoking a leaked token actually removes an open tab's on-screen names (distinct from a 503, which keeps the last frame — security); poll fetch gained an **in-flight guard + `AbortController`** (no stacking / out-of-order on half-open venue wifi — reliability + correctness); a **structural guard on the feed JSON** degrades a malformed/version-skewed payload to stale instead of throwing (never blank — TS); the duplicate-name tiebreaker uses a **stable ordinal**, not a non-unique 4-char id slice (testing); **`GridRow` memoized** with a value comparator (~11k cells stop reconciling on no-op polls — perf); added the **`board` CLI command** (agent-native read parity, no `token-mint --force` needed to inspect). Full suite green (2326 pass; the 2 pre-existing `artifacts/` ENOENT files aside).
- **Deferred to the dry run (per the plan's projector-scale deferral):** the ~90-row grid GEOMETRY on a real screen; the per-poll full event re-scan at 90 students (the new index removes the per-poll sort; `FW_MAX_PAGES=16` ≈ 16k events is ample for realistic Boston volume — measure at the 90-student seed); adv-fw6-03's benign offset-pagination-vs-live-undo grid blip (self-heals next poll). Built the geometry for 90; noted as the dry-run confirmation, exactly as Unit 5b noted its bridge-session surface gap.
- **Production residue (removable):** one live board token on `rehearsal-unit4` (auto-expires 2026-08-17); the `(cohort_id, id)` index is a permanent, harmless addition.
...
**Requirements:** FW-R25–R29, FW-D6, FW-D10–D11, FW-D13; Decisions 4, 5, 16; gaps G5, G17, G18. **PROPOSED-2 counter — ACCEPTED 2026-07-24; a committed requirement of this unit.**
...
- [x] **Unit 7: Bulk import** (paper backfill CUT — PROPOSED-3 rejected) — landed 2026-07-24. **TWO migrations**, both APPLIED to production and recorded (`20260803120000_fw_import_exceptions`, then `20260803130000_fw_import_exceptions_band` — the review-fix widening). Latest recorded version is **20260803130000**. Verified live against production via `npm run fw -- import`: a Boston-shaped CSV with deliberate dirt → 2 minted + 2 returners linked (each a second membership on ONE account, proving PROPOSED-1) + 1 within-file duplicate collapsed + 3 rows rejected per-row; a re-run minted zero; `--dry-run` and `import-exceptions` (the G7 exit-code gate) both confirmed live. Measured **~725 ms/row** → chunk of 8 ≈ 5.8 s, well inside `maxDuration = 60`. Implementation-time resolutions:
- (a) **CSV column contract (the deferred-to-implementation unknown), settled:** a header row is REQUIRED; columns match case/separator-insensitively against alias sets — a first-name column, a last-name column, and a BAND SOURCE that is a `grade` column (the realistic registration-export shape; band derived via the Path's own `bandForGrade` — no second band-derivation path) or, only when there is no grade column, an explicit `band` column. Grade wins when a file has both (a file-level, not per-row, decision). Extra columns (email, homeroom) are ignored. A header missing any of the three, or one where two columns claim the same field, is a FILE-level refusal (`FwImportParseError`), distinct from a bad DATA row (rejected alone, file continues — G19). Grade parsing accepts "7"/"6th"/"grade 6"/"6th grade" but REJECTS a decimal or trailing garbage ("12.5", "Grade 6 (transfer)") — band is identity-load-bearing.
- (b) **Identity tuple is `(normalized_name, band)`** everywhere — within-file dedupe collapses only same-name-same-band rows; the per-row match links a single same-band returner, and anything less certain parks as an exception. `normalized_name` is the shared `fwMatchKey` (`buildNormalizedFwName`), never a second key.
- (c) **Exceptions table shape** (`path_fw_import_exceptions`): student-shaped fields (`first_name`, `last_name`, `band` CHECK, `normalized_name`) + `cohort_id` scope (RESTRICT) + `reason` + `state` (`pending`/`resolved`/`dismissed`, default pending) + `created_by`/`created_at` + `resolved_at`/`resolved_by`. Indexes: the G7 pending-name lookup `(normalized_name) where pending`, the cohort open-list `(cohort_id) where pending`, and the idempotent-park unique `(cohort_id, normalized_name, **band**) where pending` (band added by the review-fix migration). RLS on, zero policies.
- (d) **Reused, never forked:** `matchFwStudent`/`fwMatchKey`/`loadFwMatchCandidates` (widened to surface pending exceptions for G7), `provisionFwStudent` + `verifyFwStudentLegs` (the no-auth-mail choke-point + compensable per-row posture — every account is password-less, `email_confirm: true`, released-alias-excluded), `linkFwStudentToCohort` (Unit 5b, with its anonymized guard), `fetchAllRows`/`fwRead`/`fwWrite`, and `isUniqueViolation` (relocated to `fw-call.ts`). No consent attestation is stamped (PROPOSED-3 cut); the retention-policy text remains due before the ~90-account Boston mint (Operational Notes).
- (e) **Chunked/compensable/resumable:** the client parses (pure code) and drives one Server-Action chunk at a time; each chunk is idempotent (a re-run skips already-minted rows via the name-in-cohort dedupe), so a timed-out chunk is simply re-sent. A crash between membership and materialization is finished in one pass (`resumed` outcome). CLI drives the SAME core (agent-native parity) and is how per-row timing was measured.
- **Review-driven changes (13 personas; 2 P1s + P2/P3s applied, 2 findings deferred):**
  - **P1 (correctness + data-migration + adversarial) — band-blind `skip_pending_exception`.** The pending-exception short-circuit keyed on `(cohort, name)` only, so a same-name-different-band child was silently skipped — never minted/linked/parked, invisible to the G7 gate; the DB unique index had the same gap. Fixed at BOTH layers (app filter `&& c.band === band`; migration `20260803130000` widens the index to include band). Compounded: `docs/solutions/logic-errors/idempotency-key-unique-scope-…` (2nd instance).
  - **P1 (adversarial) — CSV stray-quote cascade.** A stray mid-field quote flipped the tokenizer into quote mode and swallowed the rest of the roster into one rejected record. Fixed: open a quoted field only at field start (RFC 4180) + expose a `sourceDataLineCount` the UI/CLI warn on. Compounded: `docs/solutions/logic-errors/csv-tokenizer-quote-mode-on-any-quote-…`.
  - **P2s applied:** the action's throwing `buildNormalizedFwName` recompute (rejected a whole chunk) → leaner `FwImportRowInput`, core recomputes with null-safe `fwMatchKey`; a NUL byte in `identityKey`; the forked `isUniqueViolation` → `fw-call.ts`; `FwQuickCreate`'s dead "Open" link for a pending-exception match → non-clickable; the exception `reason` threaded to the outcome + a reason-copy in the ops list; decimal-grade rejection; a `resumed` outcome kind; the exception report itemizes `skipped_pending_exception`. **CLI:** `import --dry-run`, a `--chunk` value guard, friendly parse-error/`not_open` copy, an exit breadcrumb on partial import, echoed resolve inputs. **Test-harness hardening:** throw / read-error / landed-but-misreported injection + a 1000-row unranged cap; new coverage for `unexpected_error`, `match_unavailable`, the park post-write-verify branch, link failure, CRLF + doubled-quote parsing, different-band exception coexistence, `includeResolved`, dismiss, pagination. `parkFwImportException` unexported (tested through the fold). Both P1 fixes mutation-checked.
  - **Deferred (pre-existing, out of Unit 7's diff — `provision-core.ts`, unchanged since T1):** its write chain predates the `fwRead`/`fwWrite` timeout guard, and its mint-path compensation doesn't post-write-verify a landed-but-reported-failed profile insert (reliability P1/P2; adversarial concurrent-mint P2). Failure mode is a slow import a re-run completes, not data loss. **Boston-hardening follow-up** — carry into Unit 9 or a dedicated pass before the ~90-account mint.
- **Production residue (removable):** cohort `rehearsal-unit4-second` now holds 4 imported rehearsal students (Alba/Bruno Importer-Rehearsal minted; José Álvarez-Rehearsal + Jean-Luc Dubois-Rehearsal linked as second memberships) — all `-Rehearsal`-suffixed so no real student's name-derived address is burned. `path_fw_import_exceptions` is empty in production (the rehearsal import raised no exceptions).
...
| Migration mis-apply via Management API (no local DB; first execution is production) | Low | High | The documented playbook (precheck → apply → verify → record), idempotent DDL, Path suites re-run post-migration. **The "hold the apply" mitigation is retired** (Chicago cancelled) — migrations now apply as authored, which trades a revision window for the mitigation that actually catches mis-applies: each file's own pre/post-apply checklist, run in the same session |
...
- **Chicago (Jul 24–26): CANCELLED (2026-07-23).** The rehearsal, and with it the ~Jul 28 revision checkpoint, is off. **Consequence for this plan: no migration is ever held.** Every FW migration applies to production as soon as it is authored, via the Management API playbook (precheck → apply → verify → record). Two things the rehearsal was carrying now have no owner and are called out rather than dropped: the paper-flow shadowing that would have validated the check-in loop against a live room, and the **done-when spot-audit's observed working set** (the Aug 17 dry-run gate cites "Chicago's observed working set" — it now has none, so that audit runs against the plan's own Stage-1–2 task list instead).
