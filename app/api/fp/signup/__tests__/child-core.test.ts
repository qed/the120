...
function build(cfg: Cfg = {}) {
  ...
  const handle = (s: State): Result => {
    ...
    if (s.table === "fp_parental_consent") {
      if (s.op === "update") return { data: cfg.claimRows ?? [claimRow], error: null };
      return { data: cfg.existingRows ?? [], error: null }; // classify select
    }
    if (s.table === "path_role_grants") {
      // ensurePathFamilyForParent: adopt an existing family by grant.
      return { data: { scope_id: "fam1" }, error: null };
    }
    ...
    if (s.table === "fp_player_profiles") {
      if (s.op === "insert") {
        return cfg.playerInsertError
          ? { data: null, error: { code: "XX000", message: "player insert boom" } }
          : { data: { id: "pp1", handle: "dana" }, error: null };
      }
      if (s.op === "delete") return { error: null };
      return { data: null, error: null }; // existing-profile read: none
    }
    if (s.table === "fp_player_saves") {
      return { error: null }; // upsert seed / compensation delete
    }
    if (s.table === "children") {
