...
function resolveSubject(
  ...
  if (TASK_SCOPE_KINDS.has(kind)) {
    ...
    return title ? { scope: "task", id: taskId, title, href: `/path/task/${taskId}` } : null;
  }
  const criterionId = row.scopeId ?? params.criterionId;
  if (!criterionId) return null;
  const title = resolvers.criterionTitle(criterionId);
  return title ? { scope: "criterion", id: criterionId, title, href: `/path/criterion/${criterionId}` } : null;
}
