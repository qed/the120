...
import { unwrapActionResult } from "@/app/path/lib/now-card-rules";
import {
  BAND_CARDS,
  bandCardFor,
  bandVerdictForGrade,
...
import { provisionStudentAction } from "@/app/path/lib/actions/provision";
import { createFounderAction } from "@/app/path/lib/actions/onboarding";
...
export function AddFounder({
  ...
  return (
    <div className="mx-auto max-w-md pt-6">
      ...
      <h1 className={headingCls}>Who&apos;s starting The Path?</h1>
        ...
        <>
            ...
            <div className="mt-4 space-y-2">
              {needsGrade.map((f) => (
                ...
                >
                  ...
                  your 120 dashboard, then come back. The Path covers Grades 3–12.
                </p>
              ))}
            </div>
          )}
          ...
          {provisioned.length > 0 && (
            <p className="mt-4 font-path-body text-xs leading-5 text-hq-ink-muted">
              Already on The Path: {provisioned.map((f) => f.firstName).join(", ")}.
            </p>
          )}
