...
describe("scope decision parity (the System-Wide Impact decision, pinned)", () => {
  ...
  it("the apple-touch-icon file convention exists in the /path segment (180×180 PNG)", () => {
    const file = readFileSync(resolve(__dirname, "../../../fp/apple-icon.png"));
    expect(file.subarray(0, 8)).toEqual(Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]));
  ...
  it("PATH_MANIFEST_URL names the public file the /path layout links", () => {
    ...
    const layout = readFileSync(resolve(__dirname, "../../../fp/layout.tsx"), "utf8");
...
describe("sw.js delivery headers (next.config.ts)", () => {
  ...
  // The PathPwa registration pin lived here. v3 Unit 10 deleted the First
  // Profit UI (`app/fp/components/pwa/PathPwa.tsx` with it), so the ONE
  // remaining registrar is FwPwa, pinned by the set-equality check below and
  // by its own assertions further down.
...
/** `app/lib/fp/__tests__/` → the repo root. Four levels up, from THIS file. */
 ...
 * directory prefix.
 *
 * There were TWO: `PathPwa.tsx` registered the same worker at the wider `/fp`
 * scope for the First Profit app, and `FwPwa.tsx` registers it at the Founders
 * Weekend scope. v3 plan Unit 10 deleted the First Profit UI, so exactly ONE
 * registrar is left — and the list is still asserted by SET EQUALITY rather
 * than by a directory prefix, because equality is what catches a SECOND
 * registrar reappearing anywhere, including inside `app/fp/fw/**` where a
 * prefix check would have permitted it.
 */
const ALLOWED_REGISTRARS = ["app/fp/fw/components/FwPwa.tsx"];
...
describe("service-worker registration is confined to the two PWA components (Unit 5, R18)", () => {
  ...
  it("FwPwa registers at FW_SW_SCOPE with updateViaCache 'none' and the hostname guard", () => {
    ...
    const pwa = stripComments(
      readFileSync(resolve(TEST_DIR, "../../../fp/fw/components/FwPwa.tsx"), "utf8")
    );
