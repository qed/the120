...
import { skinClass, type Skin } from "@/app/fp/lib/skin-tokens";
...
function navItemsFor(skin: Skin): ShellNavItem[] {
  ...
  return skin === "trail"
    ? [
        { href: "/fp", label: "Territory Map" },
        { href: "/fp/now", label: "Your step" },
        { href: "/fp/notifications", label: "Your news" },
      ]
    : [
        { href: "/fp", label: "Dashboard" },
        { href: "/fp/now", label: "Current Task" },
        { href: "/fp/notifications", label: "Notifications" },
      ];
}

function isActive(pathname: string, href: string): boolean {
  if (href === "/fp") return pathname === "/fp";
  if (href === "/fp/notifications") return pathname.startsWith("/fp/notifications");
  // /fp/now is the stable alias for the current task/criterion surfaces.
  return pathname.startsWith("/fp/now") || pathname.startsWith("/fp/task") || pathname.startsWith("/fp/criterion");
}
      ...
      <div className={cn("min-h-screen lg:flex", canvas)}>
        ...
        <aside className={cn("hidden w-[236px] flex-shrink-0 flex-col border-r px-4 py-5 lg:flex", border, surface)}>
          <div className="flex items-center gap-2.5 px-2 pb-4">
            {brand}
            <div>
              <div className={cn("font-path-display text-[15px] font-semibold leading-none", ink)}>First Profit</div>
              <div className={cn("mt-0.5 text-[10.5px]", inkSoft)}>{roleLabel}</div>
            </div>
          </div>
          <nav className="flex flex-col gap-0.5" aria-label="First Profit">
            {nav.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  {item.label}
                  {item.href === "/fp/notifications" && unseenNews > 0 && (
                    <span
