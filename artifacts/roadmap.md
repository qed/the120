...
Remaining: (a) Gmail "Send mail as" for admissions@ (Peter, in Gmail settings — no verification needed); (b) custom SMTP for Supabase auth emails + **re-enable email confirmations** (`supabase/config.toml` → `enable_confirmations = true`, `supabase config push`) — signups are currently auto-confirmed because the default sender is rate-limited ~2/hr; (c) decide canonical domain (redirect jointhe120.vercel.app → the120.school) + update share-card URL — small dev ticket; (d) SPF/DKIM/DMARC records for the120.school outbound deliverability (quick DNS adds, worth doing before any volume email); (e) defensive domains (the120.ca, 120.school) if desired — both still available 2026-07-12.
