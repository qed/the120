...
Remaining: (a) T+2d/T+5d/T+9d account-created sequence + deposit-paid sequence (needs scheduled sends — cron ticket); (b) production E2E of the signup→welcome-email trigger (fresh signup post-deploy). Note: API key was shared in chat during setup — rotate at leisure (Resend → API keys → delete + recreate, re-paste into Vercel + Supabase SMTP).
...
Remaining: (b) custom SMTP for Supabase auth emails — 🟡 in progress: Resend SMTP chosen (`smtp.resend.com:465`, user `resend`, sender admissions@the120.school); blocked on a Supabase dashboard re-login (Peter), then key paste + save + delivery test. ⚠️ **Do NOT re-enable email confirmations as a pure config flip**: the signup flow (`AccountModal`) assumes an immediate session (profile upsert + welcome email fire right after `signUp`) — with confirmations on there's no session until the link is clicked, so that needs its own dev ticket first. (d) defensive domains (the120.ca, 120.school) if desired — both still available 2026-07-12. Housekeeping: MX record TTL is 6060s (typo, harmless) — set to 60/3600 whenever touching DNS next.
