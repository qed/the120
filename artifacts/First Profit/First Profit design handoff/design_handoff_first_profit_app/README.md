# Handoff: First Profit — the full app (Student, Parent, Guide + celebrations, docs, states)

## Overview
First Profit is the progress engine for The 120's entrepreneurship curriculum: **5 phases → 25 criteria → 125 unit tasks**, all done in the real world and verified by a real adult. This package is the **complete first-pass visual + interaction design** for the app across every role and surface:
...
The reference app the design system was reverse-engineered from is **Vite + React + TypeScript + Tailwind** — if you're building there, mirror that. If starting fresh, React + TS is the natural choice given the component library below. The prototype itself is authored as a single "Design Component" HTML file that composes the bound **First Profit Design System** (a compiled component bundle + CSS tokens). In production you would implement the real React components (contracts listed under **Design System** below) and the data/engine from the briefs.
...
Open `First Profit.dc.html`. A left **prototype navigator** (dark rail — this is scaffolding, NOT part of the product) switches between every surface. Product chrome is what's *inside* the stage:
...
1. Welcome — logo, "Welcome to First Profit", the five phase dots (Sell·Build·Validate·Grow·Scale), "Set up your family".
...
- **Tier 4 · First Profit** — grand gold panel: SCALE seal, "A year, sealed five times over", four totals ($412 / 63 customers / 47 no's / 25 crests), "a physical seal & letter from The 120 ships home".
...
## Files
- `First Profit.dc.html` — the full interactive prototype (all 18 surfaces, both skins, phone + desktop, wired loop & review). This is the primary reference.
- `assets/` — logo + evidence photography referenced by the design.
- `briefs/first-profit-app-design-brief.md` — the product brief: two-skin system, states, celebration tiers, roles, data model (§10), engine rules (§9), AI layer (§12), notifications (§13). **Source of truth for behavior.**
- `briefs/first-profit-home-study-curriculum-brief.md` — the 5 phases, 25 criteria, all 125 tasks with band variants and Done-when lines. **Source of truth for content.**
- The bound design system lives in the project at `_ds/first-profit-design-system-05a7ecff-cb52-428c-a104-274101333e3a/` (compiled bundle `_ds_bundle.js` + `tokens/*.css` + `base.css` + `components.css`). Rebuild the components listed above from these tokens.
