...
/* ─── the second-parent invite tests are GONE with the feature (v3 Unit 10) ──
   `canInviteCoParent`, `inviteVerdict` and `PARENT_INVITE_TTL_MS` had no live
   caller once the invite UI and its action module were deleted, so the logic
   and these tests went together. `normalizeEmail` below OUTLIVED them: its
   callers are the Founders Weekend guide core and action, which are live. */
