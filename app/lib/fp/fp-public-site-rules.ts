...
/* ------------------------------------------- the parent's take-offline view */

/**
 * ONE child's public page as the PARENT dashboard renders it (v3 plan Unit 10
 * follow-up: the take-offline control returns, on `/dashboard`). Produced by
 * `app/lib/fp/fp-site-parent-core.ts`, handed to the client component as a
 * prop.
 *
 * It lives in THIS pure module and not in the core because the consumer is a
 * `"use client"` component: importing a type from a `server-only` module drags
 * that module into the browser bundle, and `server-only` throws there.
 */
export type ParentSiteRow = {
  childId: string;
  firstName: string;
  handle: string;
  status: SiteStatus;
  /** Surfaced separately from `status` because the ladder deliberately folds
   *  an operator takedown and a parent takedown into the same `offline` —
   *  the PARENT is allowed to know which, because only one of them is theirs
   *  to undo. */
  operatorLocked: boolean;
};

/**
 * WHICH CONTROL THE PARENT IS OFFERED — pure, so the one branch that carries a
 * security promise is executable in the node-only harness.
 *
 * ⚠ `locked` OFFERS NO BUTTON, AND THAT IS THE POINT. An operator lock always
 * wins over a parent republish (`fp-site-ops-core`: only that path clears it,
 * and `deriveSiteStatus` keeps a locked page `offline` whatever `published`
 * says). Offering a button that flips a flag and changes nothing visible would
 * be a lie told by the UI; the core refuses to be fooled either way, so this is
 * belt AND braces rather than the only guard.
 *
 * `none` covers a page that has never been published (`claimed`): a parent
 * RESTORES a page they already launched, they do not launch one. The core
 * enforces that independently as `never-published`.
 */
export type ParentSiteControl = "take-offline" | "put-back-online" | "locked" | "none";

export function parentSiteControl(row: {
  status: SiteStatus;
  operatorLocked: boolean;
}): ParentSiteControl {
  if (row.operatorLocked) return "locked";
  if (row.status === "published") return "take-offline";
  if (row.status === "offline") return "put-back-online";
  return "none";
}

/** The status sentence beside each page. Product copy: no em dashes. */
export function parentSiteStatusLine(row: {
  status: SiteStatus;
  operatorLocked: boolean;
}): string {
  switch (parentSiteControl(row)) {
    case "locked":
      return "Taken offline by The 120. Reply to any email from us and we will explain why.";
    case "take-offline":
      return "Live. Anyone with the link can see it.";
    case "put-back-online":
      return "Offline. Only you can put it back.";
    default:
