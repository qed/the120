...
interface SealProps {
  ...
  sealed?: boolean;
  date?: string;
  /** play the wax-press entrance (used in Tier 3 celebration) */
  animate?: boolean;
  className?: string;
}

/**
 * Seal — the phase achievement mark. The report-card stamp.
...
export function Seal({
  ...
  return (
    <div className={cn('inline-flex flex-col items-center gap-1.5', className)}>
      ...
      {date &&
      <span className="font-mono text-[11px] text-hq-ink-muted">{date}</span>
      }
    </div>);
