...
import { bandVerdictForGrade, resolveSiblingAdoption } from "@/app/fp/lib/onboarding-rules";
import { normalizeStudentName, validateStudentPassword } from "@/app/fp/lib/provision-rules";
import { isParentOfFamily } from "@/app/fp/lib/provision-rules";
import { provisionStudent } from "@/app/fp/lib/provision-core";
...
export async function createFounderAction(input: unknown): Promise<CreateFounderActionResult> {
  ...
  if (!bandVerdict.ok) {
    return { success: false, error: "First Profit covers Grades 3–12 — pick a grade in that range." };
  }
