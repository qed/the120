...
**Open carries recorded at T1 close (2026-07-23; the residual queue from the sixteen unit reviews — full per-unit blocks in the T1 plan):**
- **[design → Peter, before any T2 UI ships]** Trail pending-Seal/locked-Crest neutrals sign-off (T1 U13, still open); `TrailStep` conflating `not_yet` with `submitted` shimmer; the Trail chime is user-activation-gated WebAudio (silent before first gesture) — confirm the posture on real devices.
- **[real-device half, gates T2's own exit]** The Unit 11 iPhone checklist handed to Peter: install sheet, 400 MB IDB spike, airplane-mode capture, 40 MB mid-upload resume, phone-shell pass, OS-level reduced-motion.
- **[celebration surface (U16 residuals) — natural homes: T2's Tier 2/3 + skin-toggle units]** Two-tab replay double-play (per-tab refs; needs BroadcastChannel-class coordination); display-time supersede TOCTOU (a reversal landing mid-replay still plays — the correction then surfaces on feed/task page; a per-display re-check is the fix if wanted); `TaskVerifiedMoment` hosts ALL moment tones despite its task-verified name — rename/extend when Tier 2–3 land; tone→icon map and the `/path/notifications` route string each spelled in two files; `FeedItem`'s tone/correction invariant is convention, not a discriminated union; FEED_ROW_CAP=400 with no older-items-exist indicator; `criterion_returned` event params still lack `returnedTaskIds` (the supersede pairing works by superseded_at proximity — storing the ids write-side would make it exact).
- **[product UX]** Twin same-name confirm UX on the create path ("we found an existing Alex — is this them?" — T1 U15 carry, untouched since).
- **[scale / before real cohorts]** Normalized-name column + index for `signInStudent` (U6); durable rate-limit store (U6); split the IndexedDB entry into immutable Blob + mutable progress records (U11); queue byte/count ceiling (U11); split `notify/send.ts` before new notification kinds (U12); `escapeHtml` hoist to a neutral module (U12).
- **[TP-1 compliance scope]** `path_notification_sends` + invite rows carry PII inside the future deletion scope; sign-out at-rest queue residue policy; per-student event backlog >400 has no truncation surfacing.
- **[docs]** The `export type`-in-`"use server"` question: journey-read's comment claims even local declarations crash; the dated solutions doc proves only the re-export form — reconcile when next touched (7 action files use local declarations in production without incident).
