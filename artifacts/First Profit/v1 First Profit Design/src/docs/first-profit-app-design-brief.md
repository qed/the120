---
date: 2026-07-21
topic: first-profit-app-design-brief
status: draft for review
source-of-truth: artifacts/first-profit-home-study-curriculum-brief.md (the 125 unit tasks); artifacts/Design & Marketing/2026-27 Page Handoff/design_handoff_2026-27_program_page/program-data.js (pathSteps — the 25 pass criteria)
audience: Claude Design (first pass — visual system, screens, both skins), then Claude Code CLI (build)
---

# First Profit — App Design Brief
...
First Profit is its own app with its own feel. It is not The Gauntlet (The 120's fast-math game) and should not borrow its boss-battle register. Eventually both live as tiles on a shared 120 dashboard; First Profit must stand alone until then.
...
| D10 | Wisdom: a **mix** of real quotes from real founders/investors and original lines in The 120's voice; **contextual** (keyed to where the student is on First Profit) with a light ambient layer; **collectible** into the student's Founder File. |
...
## 1. What First Profit Is

A student opens First Profit and sees exactly one thing clearly: **where they are, and what the next step is.** Behind that simplicity sits the full structure — 5 phases, each holding 5 criteria, each criterion holding 4–6 sequential unit tasks, every task ending in a binary *Done when* line that a real adult answers yes or no.
...
4. **Celebrate in proportion** — small for a task, meaningful for a criterion, big for a phase, enormous for First Profit itself.
...
**Mastery pace, no calendar shame.** First Profit is paced by mastery, not weeks. The app never compares students to each other (no cross-student leaderboards, ever) and never punishes slow weeks. Momentum nudges exist but are tied to the family's own chosen rhythm, not an absolute clock.
...
Completing Phase 05 completes First Profit: the AI assembles the full **Founder Portfolio** from the entire Founder File, and the app's largest moment plays.
...
**Key renderings.**
- Home is a **progress ledger**: First Profit as five phase rows, each showing criteria as five segments, with a single prominent "Now" card for the current task. A subtle overall meter: `37 / 125 verified`.
...
**Delivery — contextual first (D10).** The primary trigger is position on First Profit: rejection wisdom during the No Log (1.3), pricing wisdom entering the pricing experiment (3.2), delegation wisdom at 5.2, "systems over hustle" at 5.3. Rules: at most one contextual card per task; the card arrives *after* a meaningful moment (task verified, criterion opened), never as a gate before work; a small ambient pool surfaces occasionally on the home screen for variety. Wisdom never interrupts evidence capture or review.
...
4. **Founder Portfolio (once).** First Profit-complete document: the whole year, every crest, the real totals (sales, customers, no's, revenue), a curated evidence gallery, and the student's final page as its closing words. Structured to double as the completion credential.
...
**To Claude Code CLI (second):** Part Two is the contract. Build order suggestion: content package + progress engine (§8–9) → evidence pipeline (§11) → verification & review flows with notifications (§9, §13) → celebrations → AI layer (§12) → wisdom (§6) → Field Guides/math gate (§15). The 125 tasks ingest from `artifacts/first-profit-home-study-curriculum-brief.md`; the 25 pass criteria and kid register pattern from `program-data.js` (`pathSteps` / `pathStepsKid`). Validate ingestion against the totals: 25/26/24/25/25 tasks per phase, 125 total, 25 criteria, 5 phases.

**The one-sentence test for every future decision:** does this make a real thing a child did in the real world more visible, more verified, or more celebrated? If not, it doesn't belong in First Profit.
