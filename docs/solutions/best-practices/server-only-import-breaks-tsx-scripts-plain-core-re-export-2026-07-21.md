...
- **Third member of the module-boundary family (2026-07-22):** `docs/solutions/runtime-errors/use-server-type-reexport-registers-server-reference-referenceerror-2026-07-22.md` — even a TYPE-only re-export (`export type { X }`) from a `"use server"` file gets a `registerServerReference` emitted for it and throws `X is not defined` at module load, taking every action in the graph down.
